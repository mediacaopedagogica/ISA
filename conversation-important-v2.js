...
(function(){
  ...
  function css(){if($('importantBoardCss'))return;const l=document.createElement('link');l.id='importantBoardCss';l.rel='stylesheet';l.href='./conversation-important-v2.css?v=1-share-move';document.head.appendChild(l)}
  ...
  function board(){let o=$('isaImportantBoard');if(o)return o;css();o=document.createElement('section');o.id='isaImportantBoard';o.innerHTML=`<div class="ib-shell"><header class="ib-head"><span style="font-size:26px">📌</span><div class="grow"><h2>Conversa importante</h2><small>Seus post-its pessoais desta conversa. Compartilhe só quando quiser.</small></div><button class="ib-close" type="button">✕</button></header><div class="ib-tools">${Object.entries(CATS).map(([k,v])=>`<button class="ib-cat" data-ib-filter="${k}" type="button">${v[0]} ${v[1]}</button>`).join('')}<button class="ib-new" type="button">＋ Nova nota</button></div><div class="ib-pin-picker"><small>Fixador:</small>${PINS.map(p=>`<button type="button" data-ib-pin="${p}">${p}</button>`).join('')}</div><div class="ib-board"></div><div class="ib-share-hint">💜 <b>Compartilhar</b> envia uma cópia para a conversa e mantém o post-it aqui. <b>Mover para chat</b> envia e tira a nota do seu quadro pessoal.</div></div><div class="ib-editor"><div class="ib-editor-card"><h3>Nova nota ✨</h3><textarea maxlength="600" placeholder="Escreva o que quer lembrar..."></textarea><div class="ib-editor-row">${Object.entries(CATS).map(([k,v])=>`<button type="button" data-ib-cat="${k}">${v[0]} ${v[1]}</button>`).join('')}</div><div class="ib-editor-actions"><button type="button" data-ib-cancel>Cancelar</button><button type="button" class="save" data-ib-save>Salvar</button></div></div></div>`;document.body.appendChild(o)
    ...
    qa('[data-ib-pin]',o).forEach(b=>b.onclick=()=>{chosenPin=b.dataset.ibPin;qa('[data-ib-pin]',o).forEach(x=>x.classList.toggle('active',x.dataset.ibPin===chosenPin))})
  ...
  function render(){const o=board(),box=q('.ib-board',o),rows=load();box.innerHTML=rows.filter(n=>!filter||n.cat===filter).map((n,i)=>{const c=CATS[n.cat]||CATS.remember;return `<article class="ib-note" data-note="${n.id}" data-cat="${n.cat}" style="left:${Number(n.x)||20}px;top:${Number(n.y)||20}px;--rot:${Number(n.rot)||0}deg"><span class="ib-pin">${n.pin||defaultPin()}</span><button class="ib-note-menu" data-note-edit="${n.id}" type="button">•••</button><h4>${c[0]} ${c[1]}</h4><textarea readonly>${String(n.text||'').replace(/&/g,'&amp;').replace(/</g,'&lt;')}</textarea><div class="ib-note-actions"><button data-note-share="${n.id}" type="button">↥ Compartilhar</button><button data-note-move="${n.id}" type="button">💬 Mover para chat</button><button data-note-delete="${n.id}" type="button">✕</button></div></article>`}).join('')||'<div style="padding:34px;text-align:center;color:#95839d">Nenhum post-it aqui ainda. Crie uma nota para esta conversa 💜</div>'}
