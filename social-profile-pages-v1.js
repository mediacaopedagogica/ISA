...
const INDEX_TTL=5*60*1000,SIGNED_TTL=50*60*1000,signedCache=new Map()
...
async function signed(path){
  if(!path)return''
  const now=Date.now(),key='isa:signed:social-media:'+path
  let hit=signedCache.get(path)
  if(!hit){try{const raw=sessionStorage.getItem(key);if(raw){const parsed=JSON.parse(raw);if(parsed?.url&&Number(parsed.expiresAt)>now+60000)hit=parsed}}catch{}}
  if(hit?.url&&Number(hit.expiresAt)>now+60000){signedCache.set(path,hit);return hit.url}
  const {data}=await db.storage.from('social-media').createSignedUrl(path,3600),url=data?.signedUrl||''
  if(url){const entry={url,expiresAt:now+SIGNED_TTL};signedCache.set(path,entry);try{sessionStorage.setItem(key,JSON.stringify(entry))}catch{}}
  return url
}

async function refreshIndex(force=false){
  if(!force&&Date.now()-lastIndex<INDEX_TTL&&index.profiles.length)return index
  if(external()){
