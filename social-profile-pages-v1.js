...
function ensureCss(){if($('socialProfilePagesCss'))return;const l=document.createElement('link');l.id='socialProfilePagesCss';l.rel='stylesheet';l.href='./social-profile-pages-v1.css?v=1';document.head.appendChild(l)}
...
async function refreshIndex(force=false){
  ...
  if(external()){
    ...
    const {data:sp,error}=await db.from('social_profiles').select('member_id,display_name,bio,status_text,mood_emoji,activity_label,social_avatar_ref,profile_details,family_members!social_profiles_member_id_fkey(id,display_name,relationship_label)').eq('family_id',me.family_id)
    if(error)throw error
    const profiles=(sp||[]).map(p=>({memberId:p.member_id,name:p.display_name||p.family_members?.display_name||'Família',relationship:p.family_members?.relationship_label||'Família',bio:p.bio||'',statusText:p.status_text||'',moodEmoji:p.mood_emoji||'💜',activityLabel:p.activity_label||'',avatarRef:p.social_avatar_ref||'',details:p.profile_details||{}}))
...
async function fetchMainProfile(targetId){
  ...
  const {data:p,error:pErr}=await db.from('social_profiles').select('member_id,display_name,bio,status_text,mood_emoji,activity_label,theme,social_avatar_ref,status_expires_at,profile_details').eq('member_id',targetId).maybeSingle();if(pErr)throw pErr
  ...
  return{memberId:m.id,name:p?.display_name||m.display_name,relationship:m.relationship_label||'Família',bio:p?.bio||'',statusText:p?.status_text||'',moodEmoji:p?.mood_emoji||'💜',activityLabel:p?.activity_label||'',theme:p?.theme||'lilac',avatarUrl:p?.social_avatar_ref?await signed(p.social_avatar_ref):'',details:p?.profile_details||{},gallery,own:m.id===me.id}
}
async function fetchProfile(id){return external()?externalCall('profile',{targetId:id}):fetchMainProfile(id)}

function buildModal(){
  if($('socialProfilePage'))return
  const m=document.createElement('section');m.id='socialProfilePage';m.className='spp-modal';m.innerHTML=`<article class="spp-card"><header class="spp-head"><div id="sppAvatar" class="spp-avatar">💜</div><div class="spp-id"><h2 id="sppName">Perfil</h2><small id="sppRelation">Família</small><div id="sppNow" class="spp-now"></div></div><button id="sppClose" class="spp-close" type="button">✕</button></header><nav class="spp-tabs"><button class="spp-tab active" data-spp-tab="profile">👤 Perfil</button><button class="spp-tab" data-spp-tab="facts">✨ Curiosidades</button><button class="spp-tab" data-spp-tab="gallery">🖼️ Galeria</button></nav><div id="sppProfilePane" class="spp-pane active"></div><div id="sppFactsPane" class="spp-pane"></div><div id="sppGalleryPane" class="spp-pane"></div></article>`
...
function render(){
  const p=current;if(!p)return;buildModal();$('sppAvatar').innerHTML=avatarHtml(p);$('sppName').textContent=p.name||'Perfil';$('sppRelation').textContent=p.relationship||'Família';$('sppNow').textContent=`${p.moodEmoji||'💜'} ${p.activityLabel||p.statusText||'Sem atualização agora'}`
