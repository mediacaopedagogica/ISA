...
${suffix}`;
  ...
  async function __isaCore32() {
    ...
    async function chatAction(action, payload = {}) {
      ...
      if (error) throw new Error(data?.error || error.message || "Erro no chat");
    ...
    function playPing() {
      if (!["child", "super_admin", "super_parent"].includes(me?.role)) return;
      try {
    ...
    function renderNotifyBanner() {
      const allowed = ["child", "super_admin", "super_parent"].includes(me.role) && "serviceWorker" in navigator && "PushManager" in window && "Notification" in window;
    ...
    async function bindGlobalRealtime() {
      ...
      }).on("postgres_changes", { event: "*", schema: "public", table: "conversation_members" }, () => loadConversations()).on("postgres_changes", { event: "*", schema: "public", table: "conversations" }, () => loadConversations()).on("postgres_changes", { event: "*", schema: "public", table: "family_calendar" }, () => loadCalendar()).on("postgres_changes", { event: "UPDATE", schema: "public", table: "family_members" }, async () => {
        await loadFamily();
        await hydrateAvatarElements();
