/* Nuvem pin picker v2 */
(function(){
  if(window.__NUVEM_PIN_PICKER_V2__)return;window.__NUVEM_PIN_PICKER_V2__=true;
  const q=(s,r=document)=>r?.querySelector?.(s)||null,qa=(s,r=document)=>[...(r?.querySelectorAll?.(s)||[])];
  const OPT={star_blue:['★','blue','Estrela azul','star'],star_pink:['★','pink','Estrela rosa','star'],star_purple:['★','purple','Estrela roxa','star'],heart_blue:['♥','blue','Coração azul','heart'],heart_pink:['♥','pink','Coração rosa','heart'],heart_purple:['♥','purple','Coração roxo','heart'],flower:['🌸','flower','Flor','star']};
  let active=null,timer=0;
  function who(){return String(document.getElementById('myName')?.textContent||'familia').trim().toLowerCase().replace(/\s+/g,'-')}
  ...
  function css(){if(document.getElementById('nuvemPinPickerCss'))return;const l=document.createElement('link');l.id='nuvemPinPickerCss';l.rel='stylesheet';l.href='./nuvem-pin-picker-v1.css?v=2';document.head.appendChild(l)}
  function picker(){let p=document.getElementById('nuvemConversationPinPicker');if(p)return p;p=document.createElement('div');p.id='nuvemConversationPinPicker';p.innerHTML=`<div class="nuvem-pin-title">Fixar conversa com…</div>${Object.entries(OPT).map(([id,m])=>`<button class="nuvem-pin-choice" type="button" data-pin-choice="${id}">${glyph(id)}<span>${m[2]}</span></button>`).join('')}<button class="nuvem-pin-choice unpin" type="button" data-pin-choice="">✕ Desfixar conversa</button>`;document.body.appendChild(p);p.addEventListener('click',e=>{const b=e.target.closest('[data-pin-choice]');if(b&&active)choose(active,b.dataset.pinChoice||'')});document.addEventListener('pointerdown',e=>{if(p.classList.contains('show')&&!p.contains(e.target)&&!e.target.closest?.('.nuvem-pin-picker-trigger'))hide()},{capture:true});return p}
  function open(btn,card){const p=picker(),saved=read(card.dataset.conv);active=card;qa('[data-pin-choice]',p).forEach(b=>b.classList.toggle('selected',!!b.dataset.pinChoice&&b.dataset.pinChoice===saved));p.classList.add('show');const r=btn.getBoundingClientRect(),w=Math.min(p.offsetWidth||330,innerWidth-20),h=p.offsetHeight||250;let x=Math.max(10,Math.min(innerWidth-w-10,r.right-w)),y=r.bottom+8;if(y+h>innerHeight-10)y=Math.max(10,r.top-h-8);p.style.left=`${x}px`;p.style.top=`${y}px`}
  ...
  function cycle(card,count,done){const el=core(card);if(!el||count<1){done?.();return}let i=0;const go=()=>{if(i>=count){done?.();return}i++;el.click();setTimeout(go,750)};go()}
  ...
  function decorate(card){if(!card?.dataset?.conv)return;let b=q('.nuvem-pin-picker-trigger',card);if(!b){b=document.createElement('button');b.type='button';b.className='nuvem-pin-picker-trigger';b.title='Escolher como fixar esta conversa';b.onclick=e=>{e.preventDefault();e.stopPropagation();open(b,card)};card.appendChild(b)}const s=state(card),saved=read(card.dataset.conv),fallback=s==='heart'?'heart_purple':s==='star'?'star_pink':'';b.innerHTML=glyph(saved||fallback);b.classList.toggle('is-pinned',s!=='none'||!!saved)}
  function scan(){css();picker();qa('#chatList .chat-item[data-conv]').forEach(decorate)}
  const list=document.getElementById('chatList');if(list)new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(scan,100)}).observe(list,{childList:true,subtree:true,attributes:true,attributeFilter:['class','title']});
