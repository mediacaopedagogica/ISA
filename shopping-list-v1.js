...
(function(){
  ...
  function profile(){
    ...
    return allowed.has(q)?q:(allowed.has(n)?n:'')
  }
  ...
  function canManage(item){return profile()==='keise'||item.created_by===state.data?.actor?.id}

  async function api(action,payload={}){
    const {data:{session}}=await sb.auth.getSession()
    if(!session)throw new Error('Entre novamente no Cantinho para usar a Lista de Compras.')
    const {data,error}=await sb.functions.invoke('shopping-list-actions',{body:{action,...payload}})
    if(error)throw new Error(error.message||'Não foi possível atualizar a Lista de Compras.')
  ...
  function ensureModal(){
    ...
    modal=document.createElement('div');modal.id='isaShoppingModal';modal.className='hidden';modal.innerHTML=`
      <div class="isl-shell" role="dialog" aria-modal="true" aria-labelledby="islTitle">
        ...
        <div class="isl-head"><div class="isl-bag">🛒</div><div class="isl-title"><h2 id="islTitle">Lista de Compras</h2><p>Marque o que já foi comprado e combine quem vai comprar cada item.</p></div></div>
        <div id="islMembers" class="isl-members"></div><div id="islProgress" class="isl-progress"></div>
        <form id="islForm" class="isl-form">
          <div class="isl-field"><label>Item</label><input id="islName" maxlength="120" placeholder="Ex.: Leite" required></div>
  ...
  function render(){
    ...
    ensureModal();const {members,items}=state.data
    ...
    const select=$('islAssigned'),current=select.value
    select.innerHTML='<option value="">Sem responsável</option>'+members.map(m=>`<option value="${m.id}">${esc(m.display_name)}</option>`).join('')
    if([...select.options].some(o=>o.value===current))select.value=current
    const total=items.length,done=items.filter(i=>i.purchased).length,pct=total?Math.round(done*100/total):0
  ...
  function injectButton(){
    ...
    if($('isaShoppingFeatureBtn'))return true
    const b=document.createElement('button');b.id='isaShoppingFeatureBtn';b.type='button';b.className='ka-feature';b.innerHTML='<span class="ka-feature-icon">🛒</span><span>Compras</span>';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();open()},true);grid.appendChild(b);return true
  }
  ...
  window.__ISA_SHOPPING_LIST__={open,close,refresh,scan}
  ...
  scan();setTimeout(scan,800);setTimeout(scan,1900)
})();
