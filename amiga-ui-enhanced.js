...
function activeIsGroup(){const p=$('friendPhotoBtn');return !!p&&!p.classList.contains('hidden')}
function buildPlusMenu(){
  const menu=$('friendPlusMenu');if(!menu)return
  menu.innerHTML=''
  const profile=document.createElement('button');profile.type='button';profile.textContent='👤 Trocar perfil';profile.onclick=()=>{menu.classList.add('hidden');$('friendProfileEditBtn')?.click()};menu.appendChild(profile)
  if(/amiga da isa/i.test(relation)){
    const study=document.createElement('button');study.type='button';study.textContent='📚 Estudos';study.onclick=()=>{menu.classList.add('hidden');toast('Os Estudos ficam disponíveis quando este acesso estiver ativo para trabalhos escolares.')};menu.appendChild(study)
  }
  if(activeIsGroup()){
    const photo=document.createElement('button');photo.type='button';photo.textContent='📷 Imagem para atividade';photo.onclick=()=>{menu.classList.add('hidden');$('friendPhotoInput')?.click()};menu.appendChild(photo)
  }
}
...
function buildEmojiPicker(){
  const bar=$('friendEmojiBar');if(!bar||bar.dataset.enhanced==='1')return
  bar.dataset.enhanced='1';bar.classList.add('friend-emoji-picker')
  ...
  bar.querySelectorAll('[data-emoji-cat]').forEach(b=>b.onclick=()=>selectCategory(b.dataset.emojiCat))
  $('friendEmojiClose').onclick=()=>bar.classList.add('hidden')
  $('friendEmojiSearch').oninput=e=>{const q=e.target.value.trim().toLowerCase();const all=Object.values(categories).flat();renderEmojiGrid(q?all.filter(x=>emojiSearchText(x).includes(q)):all)}
...
const obs=new MutationObserver(()=>{bindPlus();bindEmoji()});obs.observe(document.documentElement,{subtree:true,childList:true})
window.addEventListener('load',()=>setTimeout(init,120));init()
