...
p = Path('app-v33.js')
s = p.read_text()
start_marker = '    async function loadConversations() {'
end_marker = '    function renderChatList() {'
start = s.rfind(start_marker)
if start < 0:
    raise SystemExit('active loadConversations not found')
end = s.find(end_marker, start)
if end < 0:
    raise SystemExit('renderChatList marker not found after active loader')

replacement = r'''    const CONVERSATION_LOAD_TIMEOUT_V58 = 5200;
...
h = index.read_text()
if 'app-v33.js?v=stable-57-conversation-boot' not in h and 'app-v33.js?v=stable-58-nonblocking-conversations' not in h:
    raise SystemExit('index app version marker not found')
h = h.replace('app-v33.js?v=stable-57-conversation-boot', 'app-v33.js?v=stable-58-nonblocking-conversations')
index.write_text(h)
print('conversation loader v58 patched')
