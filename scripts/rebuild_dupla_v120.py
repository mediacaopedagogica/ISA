...
# One continuous triangle mesh per road: no hundreds of overlapping road boxes or rail seams.
...
vehicle_pattern = r"function makePhysicsVehicle\(i\)\{.*?\}\s*vehiclePhysics\s*=\s*\[makePhysicsVehicle\(0\),makePhysicsVehicle\(1\)\];"
...
input_pattern = r"function setRaycastInput\(i,enabled,dt=\.016\)\{.*?\}\s*function zoneAt"
