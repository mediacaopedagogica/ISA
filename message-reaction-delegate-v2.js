...
if(!window.__ISA_REACTION_DELEGATE_V3__){
  window.__ISA_REACTION_DELEGATE_V3__=true
  ...
  function ensureCss(){
    ...
    const s=document.createElement('style');s.id='isaDoubleReactionCss';s.textContent=`
      #isaDoubleReactionPicker{position:fixed;z-index:2147483640;display:none;align-items:center;gap:6px;padding:8px 9px;border-radius:999px;background:rgba(255,255,255,.97);border:1px solid rgba(255,255,255,.98);box-shadow:0 14px 34px rgba(71,48,86,.23);backdrop-filter:blur(15px);-webkit-backdrop-filter:blur(15px)}
      ...
      @media(max-width:760px){#isaDoubleReactionPicker{max-width:94vw;overflow-x:auto;gap:4px;padding:7px}#isaDoubleReactionPicker button{width:35px;height:35px;flex-basis:35px;font-size:20px}}
  ...
  async function hydrateExternalIds(){
    ...
    try{
      const r=await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/rpc/friend_portal_messages`,{method:'POST',headers:{apikey:CONFIG.SUPABASE_KEY,Authorization:`Bearer ${CONFIG.SUPABASE_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({p_token:friendToken,p_conversation_id:conv}),cache:'no-store'}),list=await r.json().catch(()=>[])
