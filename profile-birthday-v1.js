...
let birthDate='',loading=null,saving=false,injectTimer=0
...
function isEditing(input){return !!input&&(document.activeElement===input||input.dataset.birthEditing==='1')}
function bindBirthInput(input){
  if(!input||input.dataset.birthBound==='1')return
  input.dataset.birthBound='1'
  input.addEventListener('focus',()=>{input.dataset.birthEditing='1'})
  input.addEventListener('input',()=>{input.dataset.birthEditing='1'})
  input.addEventListener('change',()=>{input.dataset.birthEditing='1'})
  input.addEventListener('blur',()=>{setTimeout(()=>{input.dataset.birthEditing='0'},120)})
}
function syncInputs(force=false){
  document.querySelectorAll('[data-birth-card]').forEach(card=>{
    const input=card.querySelector('[data-birth-input]');if(!input)return
    bindBirthInput(input)
    if(force||!isEditing(input)){
      const next=birthDate||''
      if(input.value!==next)input.value=next
      if(force)input.dataset.birthEditing='0'
    }
    card.classList.toggle('missing',!birthDate)
  })
}
...
async function loadBirth(force=false){
  ...
  loading=(async()=>{
    try{
      ...
      syncInputs(false);return birthDate
...
async function saveBirth(value){
  ...
  try{
    ...
    birthDate=date;syncInputs(true);toast('Data de nascimento salva 🎂');document.dispatchEvent(new CustomEvent('isa:birth-date-updated',{detail:{birthDate:date}}));return true
...
function attach(container,anchor,position='afterend'){
  ...
  const input=card.querySelector('[data-birth-input]');bindBirthInput(input);if(birthDate)input.value=birthDate
...
function scheduleInject(){clearTimeout(injectTimer);injectTimer=setTimeout(inject,120)}
...
const obs=new MutationObserver(scheduleInject);obs.observe(document.documentElement,{childList:true,subtree:true})
