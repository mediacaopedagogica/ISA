...
Suggested solution: ${env.workaround}`;
    ...
    triggerChanError(reason) {
      this.channels.forEach((channel3) => {
        if (!(channel3.isErrored() || channel3.isLeaving() || channel3.isClosed())) {
          channel3.trigger(CHANNEL_EVENTS2.error, reason);
        }
      });
    }
    /**
  ...
  var PresenceAdapter = class _PresenceAdapter {
    constructor(channel3, opts) {
      ...
      this.presence = new Presence(channel3.getChannel(), phoenixOptions);
  ...
  var serialize = (operator, value) => {
    if (operator === "in") {
      ...
      const items2 = Array.from(new Set(values)).map((v) => serializeScalar(v)).join(",");
      return `in.(${items2})`;
    }
    if (operator === "is") {
      ...
      if (this.accessTokenValue != tokenToSend) {
        ...
        this.channels.forEach((channel3) => {
          ...
          if (channel3.joinedOnce && channel3.channelAdapter.isJoined()) {
            channel3.channelAdapter.push(CHANNEL_EVENTS.access_token, {
              access_token: tokenToSend
            });
          }
        });
...
${suffix}`;
  ...
  var CONFIG = {
    ...
    VAPID_PUBLIC_KEY: "BHitKQYrrLqSBcX5cATGKWgow1iN7kstrctNKfwkqBZRIx5tt-UNpiwmCNwjhEnTkWORRXm3M_93_-WomAoxiyk",
    APP_NAME: "Cantinho da Isa"
  };

  // study-calculator.js
  var import_esm = __require("https://cdn.jsdelivr.net/npm/mathjs@14/+esm");
  function initStudyCalculator({ onSendResult }) {
    ...
    function render6() {
      if ($4("calcExpression")) $4("calcExpression").textContent = expression || " ";
      if ($4("calcResult")) $4("calcResult").textContent = result;
      renderHistory();
    }
    function setMode2(value) {
      scientific = value;
      $4("calcNormalBtn")?.classList.toggle("active", !value);
      $4("calcScientificBtn")?.classList.toggle("active", value);
      document.querySelectorAll(".calc-key.scientific").forEach((x) => x.classList.toggle("sci-hidden", !value));
    }
    function calculate() {
      if (!expression) return;
      try {
        let exp = expression.replaceAll("\xD7", "*").replaceAll("\xF7", "/").replaceAll("\u03C0", "pi").replaceAll("ln(", "log(");
        const value = (0, import_esm.evaluate)(exp);
        if (typeof value !== "number" || !Number.isFinite(value)) throw new Error();
        result = Number.isInteger(value) ? String(value) : String(Number(value.toPrecision(12)));
    ...
    $4("calcNormalBtn")?.addEventListener("click", () => setMode2(false));
    $4("calcScientificBtn")?.addEventListener("click", () => setMode2(true));
    document.querySelectorAll("#calcKeys [data-calc],#calcKeys [data-action]").forEach((b) => b.addEventListener("click", () => handle(b)));
    setMode2(scientific);
    render6();
    return { render: render6, setMode: setMode2 };
  }

  // study.js
  var db = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: true } });
  ...
  var note = null;
  var saveTimer = null;
  var noteChannel = null;
  var calculatorReady = false;
  function esc(v) {
    return String(v ?? "").replace(/[&<>'"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[c]);
  }
  function css() {
    if (document.querySelector('link[href^="study.css"]')) return;
    const l = document.createElement("link");
    l.rel = "stylesheet";
    l.href = "study.css?v=36";
    document.head.appendChild(l);
  }
  function toast(text) {
    const t = $("toast") || document.getElementById("friendToast");
    if (!t) return;
    t.textContent = text;
  ...
  function mount() {
    css();
    const content = document.querySelector(".content") || document.querySelector(".friend-main");
    if (!content) return;
    if (!$("studyPanel")) {
      ...
      p.className = "hidden";
      p.innerHTML = `
    <div class="study-head"><div><h2>Mesa de Estudos</h2><p>Use individualmente ou compartilhe com um grupo.</p></div><select id="studyScope" class="study-scope"><option value="private">S\xF3 meu</option></select></div>
    <div id="studyHome"><div class="study-home-grid"><button class="study-tool-card" id="openCalculatorTool" type="button"><span class="tool-symbol">\u{1F9EE}</span><strong>Calculadora + Caderno</strong><small>Calculadora visual normal ou cient\xEDfica e caderno digital lado a lado.</small></button></div></div>
    <div id="studyCalculator" class="study-workspace hidden">
      ...
      <div class="calc-note-layout">
        <section class="calculator-3d">
          <div class="calc-top"><strong>Calculadora</strong><div class="calc-mode"><button type="button" id="calcNormalBtn" class="active">Normal</button><button type="button" id="calcScientificBtn">Cient\xEDfica</button></div></div>
          ...
          <div id="calcKeys" class="calc-keys">
            ...
            <button class="calc-key scientific sci-hidden" data-calc="e">e</button><button class="calc-key scientific sci-hidden" data-action="factorial">x!</button><button class="calc-key scientific sci-hidden" data-calc="abs(">abs</button><button class="calc-key scientific sci-hidden" data-action="inverse">1/x</button><button class="calc-key scientific sci-hidden" data-action="square">x\xB2</button>
            <button class="calc-key operator" data-action="clear">C</button><button class="calc-key" data-action="backspace">\u232B</button><button class="calc-key" data-calc="(">(</button><button class="calc-key" data-calc=")">)</button><button class="calc-key operator" data-calc="\xF7">\xF7</button>
            <button class="calc-key" data-calc="7">7</button><button class="calc-key" data-calc="8">8</button><button class="calc-key" data-calc="9">9</button><button class="calc-key operator" data-calc="\xD7">\xD7</button><button class="calc-key operator" data-action="percent">%</button>
            ...
            <button class="calc-key" data-calc="0">0</button><button class="calc-key" data-calc=".">.</button><button class="calc-key" data-calc="\u03C0">\u03C0</button><button class="calc-key operator" data-action="negative">\xB1</button><button class="calc-key equal" data-action="equals">=</button>
          </div><div id="calcHistory" class="calc-history"></div>
        </section>
        <section class="notebook-3d"><header class="notebook-head"><div><strong id="notebookTitle">Meu caderno</strong><small>Digite ou envie resultados da calculadora.</small></div><span id="notebookStatus" class="notebook-status">Carregando\u2026</span></header><textarea id="studyNotebook" class="notebook-paper" placeholder="Comece suas anota\xE7\xF5es aqui..."></textarea><footer class="notebook-actions"><span class="grow-note">Salvamento autom\xE1tico</span><button id="saveNotebookBtn" type="button">Salvar agora</button><button id="clearNotebookBtn" type="button">Limpar p\xE1gina</button></footer></section>
      </div>
    </div>`;
      content.appendChild(p);
    }
    bind();
    window.__ISA_OPEN_STUDY__ = show;
  }
  function bind() {
    document.querySelectorAll(".nav-btn").forEach((b) => b.addEventListener("click", () => $("studyPanel")?.classList.add("hidden")));
    $("openCalculatorTool")?.addEventListener("click", openCalculator);
    $("studyBackBtn")?.addEventListener("click", () => {
      $("studyCalculator").classList.add("hidden");
      $("studyHome").classList.remove("hidden");
    });
    $("studyScope")?.addEventListener("change", async (e) => {
      scope = e.target.value;
      await loadNote();
    });
    $("studyNotebook")?.addEventListener("input", scheduleSave);
    $("saveNotebookBtn")?.addEventListener("click", () => saveNote(true));
    $("clearNotebookBtn")?.addEventListener("click", () => {
      if (confirm("Limpar esta p\xE1gina do caderno?")) {
        $("studyNotebook").value = "";
        saveNote(true);
      }
    });
  ...
  async function show(scopeHint) {
    ...
    try {
      await identity();
      const hint = scopeHint || window.__ISA_STUDY_SCOPE_HINT__;
      if (hint && hint !== "private" && groups.some((g) => g.id === hint)) scope = hint;
      else if (hint === "private") scope = "private";
      const sel = $("studyScope");
      if (sel) {
        sel.value = scope;
      }
      await loadNote();
      ...
      console.error(e);
      toast("N\xE3o foi poss\xEDvel abrir Estudos.");
    }
  }
  function openCalculator() {
    $("studyHome").classList.add("hidden");
    $("studyCalculator").classList.remove("hidden");
    if (!calculatorReady) {
      initStudyCalculator({ onSendResult: sendResult });
      calculatorReady = true;
    }
    identity().then(loadNote).catch(console.error);
  }
  async function identity() {
    if (me) return me;
    const { data: { user } } = await db.auth.getUser();
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await db.from("family_members").select("id,family_id,display_name,role,relationship_label").eq("auth_user_id", user.id).eq("active", true).single();
    if (error || !data) throw error || new Error("Perfil n\xE3o encontrado");
    me = data;
    const allowed = me.role === "child" || String(me.relationship_label || "").trim().toLowerCase() === "amiga da isa";
    if (!allowed) throw new Error("Estudos \xE9 exclusivo da Isa e amigas autorizadas");
    const { data: cm } = await db.from("conversation_members").select("conversation_id").eq("member_id", me.id);
    const ids = [...new Set((cm || []).map((x) => x.conversation_id))];
    if (ids.length) {
      const { data: g } = await db.from("conversations").select("id,title,type,updated_at").in("id", ids).eq("type", "group").order("updated_at", { ascending: false });
      groups = g || [];
    }
    const sel = $("studyScope");
    if (sel) {
      sel.innerHTML = '<option value="private">S\xF3 meu</option>' + groups.map((g) => `<option value="${g.id}">Grupo \u2022 ${esc(g.title || "Sem nome")}</option>`).join("");
      if (![...sel.options].some((o) => o.value === scope)) scope = "private";
      sel.value = scope;
    }
    return me;
  }
  async function loadNote() {
    if (!$("studyNotebook")) return;
    await identity();
    clearTimeout(saveTimer);
    note = null;
    if (noteChannel) {
      db.removeChannel(noteChannel);
      noteChannel = null;
    }
    $("notebookStatus").textContent = "Carregando\u2026";
    const group = scope !== "private" ? groups.find((g) => g.id === scope) : null;
    ...
    if (group) q = q.eq("conversation_id", group.id).eq("visibility", "group").eq("title", "Caderno do grupo \u2022 Calculadora");
    else q = q.eq("owner_member_id", me.id).is("conversation_id", null).eq("visibility", "private").eq("title", "Meu caderno \u2022 Calculadora");
    const { data, error } = await q.order("updated_at", { ascending: false }).limit(1);
    if (error) {
      console.error(error);
      $("notebookStatus").textContent = "Erro ao carregar";
      return;
    }
    note = data?.[0] || null;
    $("studyNotebook").value = note?.content?.text || "";
    $("notebookTitle").textContent = group ? `Caderno \u2022 ${group.title}` : "Meu caderno";
    $("notebookStatus").textContent = note ? "Salvo \u2713" : "Novo caderno";
    if (note) {
      noteChannel = db.channel(`study-note-${note.id}`).on("postgres_changes", { event: "UPDATE", schema: "public", table: "study_items", filter: `id=eq.${note.id}` }, (payload) => {
        if (document.activeElement !== $("studyNotebook")) {
          $("studyNotebook").value = payload.new?.content?.text || "";
          $("notebookStatus").textContent = "Atualizado \u2713";
        }
      }).subscribe();
    }
  }
  function scheduleSave() {
    if (!me) return;
    $("notebookStatus").textContent = "Salvando\u2026";
    clearTimeout(saveTimer);
  ...
  async function saveNote(feedback = false) {
    ...
    $("notebookStatus").textContent = "Salvo \u2713";
    if (feedback) toast("Caderno salvo \u2713");
  }
  function sendResult(h) {
    if (!h || !$("studyNotebook")) return;
    const ta = $("studyNotebook"), line = `${h.expression} = ${h.result}`, start2 = ta.selectionStart ?? ta.value.length, end = ta.selectionEnd ?? ta.value.length, prefix = ta.value && start2 > 0 && !ta.value.slice(0, start2).endsWith("\n") ? "\n" : "";
    ta.value = ta.value.slice(0, start2) + prefix + line + "\n" + ta.value.slice(end);
    ta.focus();
    ta.selectionStart = ta.selectionEnd = start2 + prefix.length + line.length + 1;
    scheduleSave();
    toast("Resultado enviado ao caderno");
  }
  mount();
  db.auth.onAuthStateChange((_e, session) => {
    if (!session) {
      ...
      groups = [];
      note = null;
    } else if ($("studyPanel") && !$("studyPanel").classList.contains("hidden")) identity().then(loadNote).catch(console.error);
  });

  // study-document.js
  var dbDoc = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  var $d = (id) => document.getElementById(id);
  var docMe = null;
  var activeDoc = null;
  var docSaveTimer = null;
  var docDirty = false;
  var docScope = "private";
  function escDoc(v) {
    return String(v ?? "").replace(/[&<>'"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[c]);
  }
  function addCss() {
  ...
  function waitMount() {
    if ($d("studyPanel") && document.querySelector(".study-home-grid")) return mountDocument();
  ...
  function mountDocument() {
    ...
    if (grid && !$d("openDocumentTool")) {
      ...
      b.innerHTML = '<span class="tool-symbol">\u{1F4C4}</span><strong>Documento</strong><small>Editor completo para trabalhos, com formata\xE7\xE3o, dicas, hist\xF3rico e PDF.</small>';
      grid.appendChild(b);
    }
    if (!$d("studyDocument")) {
    ...
    <div class="study-document-shell">
      <div class="doc-commandbar"><button id="docNewBtn" type="button">Novo</button><button id="docOpenBtn" type="button">Abrir</button><input id="docTitle" class="doc-title-input" value="Trabalho sem t\xEDtulo" aria-label="Nome do documento"><span id="docSaveState" class="doc-save-state">Novo</span><button id="docSaveBtn" class="doc-primary" type="button">Salvar</button><button id="docHistoryBtn" type="button">Hist\xF3rico</button><button id="docGuidedBtn" type="button">Trabalho guiado</button><button id="docHelpBtn" class="doc-tip-btn" type="button">Como fazer?</button><button id="docPreviewBtn" type="button">Ver como fica</button><button id="docPdfBtn" type="button">PDF</button><button id="docPrintBtn" type="button">Imprimir</button></div>
      <div class="doc-toolbar" role="toolbar" aria-label="Formata\xE7\xE3o do documento">
        ...
        <select id="docStyle" title="Estilo"><option value="p">Texto normal</option><option value="h1">T\xEDtulo 1</option><option value="h2">T\xEDtulo 2</option><option value="h3">T\xEDtulo 3</option><option value="blockquote">Cita\xE7\xE3o</option></select>
        ...
        <span class="doc-separator"></span><button data-doc-cmd="bold" title="Negrito"><b>B</b></button><button data-doc-cmd="italic" title="It\xE1lico"><i>I</i></button><button data-doc-cmd="underline" title="Sublinhado"><u>U</u></button><button data-doc-cmd="strikeThrough" title="Tachado"><s>S</s></button><button data-doc-cmd="superscript" title="Sobrescrito">x\xB2</button><button data-doc-cmd="subscript" title="Subscrito">x\u2082</button>
        ...
        <button data-doc-cmd="justifyLeft" title="Alinhar \xE0 esquerda">\u2261</button><button data-doc-cmd="justifyCenter" title="Centralizar">\u2263</button><button data-doc-cmd="justifyRight" title="Alinhar \xE0 direita">\u2261\u2192</button><button data-doc-cmd="justifyFull" title="Justificar">\u2630</button><button data-doc-cmd="insertUnorderedList" title="Marcadores">\u2022 Lista</button><button data-doc-cmd="insertOrderedList" title="Numera\xE7\xE3o">1. Lista</button><button data-doc-cmd="outdent" title="Diminuir recuo">\u21E4</button><button data-doc-cmd="indent" title="Aumentar recuo">\u21E5</button>
        <select id="docLineHeight" title="Espa\xE7amento"><option value="1">1,0</option><option value="1.15">1,15</option><option value="1.5" selected>1,5</option><option value="2">2,0</option></select><span class="doc-separator"></span>
        <button id="docLinkBtn" type="button" title="Inserir link">Link</button><button id="docImageBtn" type="button" title="Inserir imagem">Imagem</button><input id="docImageInput" type="file" accept="image/png,image/jpeg,image/webp" hidden><button id="docTableBtn" type="button" title="Inserir tabela">Tabela</button><button id="docBreakBtn" type="button" title="Quebra de p\xE1gina">Quebra</button><button data-doc-cmd="removeFormat" title="Limpar formata\xE7\xE3o">Limpar</button>
        <select id="docMargin" title="Margens"><option value="normal">Margens normais</option><option value="narrow">Estreitas</option><option value="wide">Largas</option></select><select id="docOrientation" title="Orienta\xE7\xE3o"><option value="portrait">Retrato</option><option value="landscape">Paisagem</option></select>
      </div>
      ...
      <div class="doc-footerbar"><span class="grow">A4 \u2022 salvamento autom\xE1tico</span><span id="docWordCount" class="doc-wordcount">0 palavras</span></div>
    </div>`;
    ...
    if (!$d("studyDocModal")) {
      ...
      d.innerHTML = '<div class="doc-modal-card"><button id="studyDocModalClose" class="doc-modal-close" type="button">\u2715</button><div id="studyDocModalBody"></div></div>';
  ...
  function bindDocument() {
    ...
    document.querySelectorAll("[data-doc-cmd]").forEach((b) => b.onclick = () => command(b.dataset.docCmd));
    ...
    $d("docBreakBtn").onclick = () => insertHtml('<div class="page-break" contenteditable="false">Quebra de p\xE1gina</div><p><br></p>');
  ...
  function openEditor() {
    ...
    identityDoc().then(() => {
      ...
      console.error(e);
      toastDoc("N\xE3o foi poss\xEDvel abrir o editor.");
    });
  }
  function backHome() {
  ...
  async function identityDoc() {
    ...
    const { data: { user } } = await dbDoc.auth.getUser();
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await dbDoc.from("family_members").select("id,family_id,display_name").eq("auth_user_id", user.id).eq("active", true).single();
  ...
  function newDocument(focus = true) {
    activeDoc = null;
    docDirty = false;
    $d("docTitle").value = "Trabalho sem t\xEDtulo";
  ...
  function scopeInfo() {
    ...
    return v === "private" ? { conversation_id: null, visibility: "private" } : { conversation_id: v, visibility: "group" };
  }
  async function saveDocument(feedback = false, version5 = false) {
    await identityDoc();
    clearTimeout(docSaveTimer);
    const si = scopeInfo(), content = { html: $d("docPage").innerHTML, margin: $d("docPage").dataset.margin || "normal", orientation: $d("docPage").dataset.orientation || "portrait" };
    setSave("Salvando\u2026");
    if (!activeDoc) {
      const { data, error } = await dbDoc.from("study_items").insert({ family_id: docMe.family_id, conversation_id: si.conversation_id, owner_member_id: docMe.id, item_type: "document", title: $d("docTitle").value.trim() || "Sem t\xEDtulo", content, visibility: si.visibility, allowed_member_ids: [], updated_by: docMe.id }).select("id,title,content,conversation_id,visibility,owner_member_id,updated_at").single();
      if (error) {
        console.error(error);
        setSave("Erro");
        return;
      ...
      const { data, error } = await dbDoc.from("study_items").update({ title: $d("docTitle").value.trim() || "Sem t\xEDtulo", content, updated_by: docMe.id }).eq("id", activeDoc.id).select("id,title,content,conversation_id,visibility,owner_member_id,updated_at").single();
      if (error) {
        console.error(error);
        setSave("Erro");
        return;
    ...
    docDirty = false;
    setSave("Salvo \u2713");
    if (version5) await saveVersion();
    if (feedback) toastDoc("Documento salvo \u2713");
  }
  async function saveVersion() {
    if (!activeDoc) return;
    const { data } = await dbDoc.from("study_item_versions").select("version_no").eq("item_id", activeDoc.id).order("version_no", { ascending: false }).limit(1);
    const n = (data?.[0]?.version_no || 0) + 1;
    await dbDoc.from("study_item_versions").insert({ item_id: activeDoc.id, version_no: n, content: { title: $d("docTitle").value, html: $d("docPage").innerHTML, margin: $d("docPage").dataset.margin, orientation: $d("docPage").dataset.orientation }, saved_by: docMe.id });
  }
  async function documentsForScope() {
    await identityDoc();
    const si = scopeInfo();
    let q = dbDoc.from("study_items").select("id,title,content,updated_at,owner_member_id").eq("item_type", "document");
    if (si.visibility === "private") q = q.eq("owner_member_id", docMe.id).is("conversation_id", null).eq("visibility", "private");
    else q = q.eq("conversation_id", si.conversation_id).eq("visibility", "group");
  ...
  async function openDocumentList() {
    try {
      ...
      openModal(`<h3>Abrir documento</h3><div class="doc-list-dialog">${docs.map((d) => `<div class="doc-list-row"><strong>${escDoc(d.title)}</strong><small>${new Date(d.updated_at).toLocaleDateString("pt-BR")}</small><button data-open-doc="${d.id}">Abrir</button></div>`).join("") || "<p>Nenhum documento salvo aqui ainda.</p>"}</div>`);
      document.querySelectorAll("[data-open-doc]").forEach((b) => b.onclick = () => loadDocument(b.dataset.openDoc));
    } catch (e) {
      console.error(e);
      toastDoc("N\xE3o foi poss\xEDvel listar documentos.");
    }
  }
  async function loadDocument(id) {
    const { data, error } = await dbDoc.from("study_items").select("id,title,content,conversation_id,visibility,owner_member_id,updated_at").eq("id", id).single();
    if (error) return toastDoc("N\xE3o foi poss\xEDvel abrir.");
    activeDoc = data;
  ...
  async function openHistory() {
    ...
    const { data, error } = await dbDoc.from("study_item_versions").select("id,version_no,content,created_at").eq("item_id", activeDoc.id).order("version_no", { ascending: false }).limit(30);
    if (error) return toastDoc("N\xE3o foi poss\xEDvel abrir o hist\xF3rico.");
    openModal(`<h3>Hist\xF3rico de vers\xF5es</h3>${(data || []).map((v) => `<div class="doc-history-row"><div><strong>Vers\xE3o ${v.version_no}</strong><small> \u2022 ${new Date(v.created_at).toLocaleString("pt-BR")}</small></div><button data-restore-version="${v.id}">Restaurar</button></div>`).join("") || "<p>Use o bot\xE3o Salvar para criar uma vers\xE3o recuper\xE1vel.</p>"}`);
    document.querySelectorAll("[data-restore-version]").forEach((b) => b.onclick = () => restoreVersion(b.dataset.restoreVersion, data));
  }
  function restoreVersion(id, list) {
    ...
    changed();
    closeModal();
    toastDoc("Vers\xE3o restaurada. Salve para confirmar.");
  }
  ...
  function insertLink() {
    const url = prompt("Cole o endere\xE7o do link:");
    if (url) command("createLink", url);
  }
  function insertImage(file) {
    if (!file) return;
    if (file.size > 1.5 * 1024 * 1024) {
      toastDoc("Use uma imagem de at\xE9 1,5 MB neste editor.");
      return;
    }
    const r = new FileReader();
    r.onload = () => insertHtml(`<p><img src="${r.result}" alt="Imagem inserida"></p>`);
    r.readAsDataURL(file);
    $d("docImageInput").value = "";
  }
  function insertTable() {
    const rows = Math.min(12, Math.max(1, Number(prompt("Quantas linhas?", "3")) || 0)), cols = Math.min(8, Math.max(1, Number(prompt("Quantas colunas?", "3")) || 0));
    if (!rows || !cols) return;
    let h = "<table><tbody>";
    for (let r = 0; r < rows; r++) {
      h += "<tr>";
      for (let c = 0; c < cols; c++) h += `<${r === 0 ? "th" : "td"}><br></${r === 0 ? "th" : "td"}>`;
      h += "</tr>";
    }
  ...
  function countWords() {
    const t = $d("docPage")?.innerText.trim() || "", n = t ? t.split(/\s+/).length : 0;
  ...
  function openHelp() {
    ...
    let tip = "Escreva com suas pr\xF3prias palavras e organize uma ideia principal por par\xE1grafo.";
    if (tag === "h1" || tag === "h2" || tag === "h3") tip = "Use t\xEDtulos curtos e claros para mostrar ao leitor o assunto da pr\xF3xima parte.";
    const text = ($d("docPage").innerText || "").toLowerCase();
    if (text.includes("introdu\xE7\xE3o") && window.getSelection()?.anchorNode?.parentElement?.closest("p")) tip = "Na introdu\xE7\xE3o, apresente o tema, d\xEA o contexto necess\xE1rio e explique o objetivo do trabalho. Evite antecipar toda a conclus\xE3o.";
    openModal(`<h3>Como fazer?</h3><div class="doc-help-box"><strong>Dica para esta parte</strong><p>${escDoc(tip)}</p><p>Se sua escola ou professora passou regras espec\xEDficas, elas t\xEAm prioridade sobre sugest\xF5es gerais de formata\xE7\xE3o.</p></div>`);
  }
  function openGuided() {
    openModal(`<h3>Criar trabalho guiado</h3><div class="doc-guided-grid"><label>T\xEDtulo<input id="guidedTitle"></label><label>Disciplina<input id="guidedSubject"></label><label>Professor(a)<input id="guidedTeacher"></label><label>Turma<input id="guidedClass"></label><label class="full">Integrantes<input id="guidedMembers"></label><label>Tipo<select id="guidedType"><option value="simple">Trabalho simples</option><option value="research">Pesquisa</option><option value="report">Relat\xF3rio</option><option value="seminar">Semin\xE1rio</option></select></label><label>Modo<select id="guidedMode"><option value="school">Escolar</option><option value="free">Livre</option><option value="abnt">ABNT orientativo</option></select></label></div><button id="guidedCreateBtn" class="primary-btn" type="button">Criar estrutura</button>`);
    $d("guidedCreateBtn").onclick = createGuided;
  }
  function createGuided() {
    const title = $d("guidedTitle").value.trim() || "T\xEDtulo do trabalho", subject = $d("guidedSubject").value.trim(), teacher = $d("guidedTeacher").value.trim(), klass = $d("guidedClass").value.trim(), members = $d("guidedMembers").value.trim(), type = $d("guidedType").value, mode = $d("guidedMode").value;
    let body = `<div class="school-cover"><div>${escDoc(subject)}</div><div class="cover-middle"><p>${escDoc(members)}</p><h1 style="text-align:center">${escDoc(title)}</h1></div><div>${escDoc(teacher)}${klass ? " \u2022 " + escDoc(klass) : ""}<br>${(/* @__PURE__ */ new Date()).getFullYear()}</div></div><div class="page-break" contenteditable="false">Quebra de p\xE1gina</div><h1>Introdu\xE7\xE3o</h1><p class="hint-text">Apresente o tema e o objetivo do trabalho. Apague esta dica quando terminar.</p><p><br></p><h1>Desenvolvimento</h1><p class="hint-text">Organize aqui as informa\xE7\xF5es pesquisadas em partes e subt\xEDtulos.</p><p><br></p>`;
    if (type === "report") body += "<h1>Resultados ou observa\xE7\xF5es</h1><p><br></p>";
    body += '<h1>Conclus\xE3o</h1><p class="hint-text">Retome o objetivo e sintetize o que foi aprendido ou encontrado.</p><p><br></p><h1>Refer\xEAncias</h1><p class="hint-text">Liste as fontes realmente utilizadas. N\xE3o invente dados que voc\xEA n\xE3o tenha.</p><p><br></p>';
    if (mode === "abnt") body = '<p class="hint-text"><strong>Modo ABNT orientativo:</strong> confira sempre as exig\xEAncias da sua escola. O editor oferece estrutura e ajuda, mas n\xE3o substitui as instru\xE7\xF5es da atividade.</p>' + body;
    $d("docTitle").value = title;
    $d("docPage").innerHTML = body;
    activeDoc = null;
    changed();
    closeModal();
    toastDoc("Estrutura criada. Agora complete com o seu conte\xFAdo.");
  }
  ...
  async function exportPdf() {
    try {
      ...
      clone.style.width = $d("docPage").dataset.orientation === "landscape" ? "297mm" : "210mm";
      clone.style.minHeight = $d("docPage").dataset.orientation === "landscape" ? "210mm" : "297mm";
      const opt = { margin: 0, filename: `${($d("docTitle").value || "trabalho").replace(/[^a-z0-9áéíóúãõç _-]/gi, "")}.pdf`, image: { type: "jpeg", quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: "mm", format: "a4", orientation: $d("docPage").dataset.orientation === "landscape" ? "landscape" : "portrait" }, pagebreak: { mode: ["css", "legacy"] } };
      await window.html2pdf().set(opt).from(clone).save();
      toastDoc("PDF gerado \u2713");
    } catch (e) {
      console.error(e);
      toastDoc("N\xE3o foi poss\xEDvel gerar o PDF neste navegador.");
    }
  }
  function printDocument() {
    const w = window.open("", "_blank");
    if (!w) return toastDoc("Permita a abertura de janela para imprimir.");
    w.document.write(`<html><head><title>${escDoc($d("docTitle").value)}</title><style>body{font-family:Arial;margin:0}.page{max-width:210mm;margin:auto;padding:20mm;line-height:1.5}.page img{max-width:100%}.page table{border-collapse:collapse;width:100%}.page td,.page th{border:1px solid #999;padding:6px}.page-break{break-after:page;border:0;color:transparent}</style></head><body><div class="page">${$d("docPage").innerHTML}</div><script>onload=()=>print()<\/script></body></html>`);
    w.document.close();
  }
  waitMount();
  ...
  function ensureRetentionCss() {
    ...
    const l = document.createElement("link");
    l.rel = "stylesheet";
    l.href = "retention-notice.css?v=1";
    document.head.appendChild(l);
  }
  var retentionTimer = null;
  function showRetentionCloud(kind = "documento") {
    ensureRetentionCss();
    let el = document.getElementById("retentionCloud");
    if (!el) {
      el = document.createElement("div");
      el.id = "retentionCloud";
      el.className = "retention-cloud";
      el.setAttribute("role", "status");
      el.setAttribute("aria-live", "polite");
      document.body.appendChild(el);
    }
    const target = kind === "apresentacao" ? "PowerPoint" : "PDF";
    el.innerHTML = `<span class="retention-dot"></span><strong>Arquivo tempor\xE1rio.</strong> Este arquivo fica salvo por at\xE9 7 dias. Baixe em ${target} para guardar uma c\xF3pia.`;
  ...
  document.addEventListener("click", (e) => {
    const doc = e.target.closest("#openDocumentTool");
    if (doc) {
      showRetentionCloud("documento");
      return;
    }
    const pres = e.target.closest("#openPresentationTool");
    if (pres) showRetentionCloud("apresentacao");
  }, { capture: true });
  window.showRetentionCloud = showRetentionCloud;

  // study-randomizer.js
  var dbR = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  var $r = (id) => document.getElementById(id);
  var meR = null;
  function addCss2() {
    if (document.querySelector('link[href^="study-randomizer.css"]')) return;
    const l = document.createElement("link");
    l.rel = "stylesheet";
  ...
  async function identity3() {
    ...
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await dbR.from("family_members").select("id,family_id,display_name").eq("auth_user_id", user.id).eq("active", true).single();
    if (error) throw error;
    meR = data;
    return data;
  }
  ...
  function mount2() {
    addCss2();
    const grid = document.querySelector(".study-home-grid"), panel = $r("studyPanel");
    if (!grid || !panel) return false;
    if (!$r("openRandomizerTool")) {
      const b = document.createElement("button");
      b.id = "openRandomizerTool";
      b.type = "button";
      b.className = "study-tool-card";
      b.style.background = "linear-gradient(145deg,#fff1bf,#ffddeb 50%,#e3d8ff)";
      b.innerHTML = '<span class="tool-symbol">\u{1F3B2}</span><strong>Sorteador</strong><small>Sorteie temas, tarefas ou a ordem da apresenta\xE7\xE3o.</small>';
      grid.appendChild(b);
    }
    if (!$r("studyRandomizer")) {
      const s = document.createElement("div");
      s.id = "studyRandomizer";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="randomBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="randomizer-shell"><section class="randomizer-card"><div class="randomizer-head"><h3>Sorteador de Temas e Tarefas</h3><select id="randomMode" class="randomizer-mode"><option value="themes">Distribuir temas</option><option value="tasks">Distribuir tarefas</option><option value="order">Ordem de apresenta\xE7\xE3o</option><option value="quick">Sorteio r\xE1pido</option></select></div><div class="randomizer-grid"><div class="randomizer-box"><label>Participantes</label><textarea id="randomPeople" placeholder="Um nome por linha"></textarea><button id="randomUseGroup" type="button">Usar integrantes do grupo</button></div><div class="randomizer-box"><label id="randomItemsLabel">Temas</label><textarea id="randomItems" placeholder="Um tema por linha"></textarea></div></div><div class="randomizer-actions"><button id="randomDraw" type="button">Sortear agora</button><button id="randomAgain" type="button">Sortear novamente</button><button id="randomClear" type="button">Limpar</button></div><div id="randomResult" class="randomizer-result"><h4>Resultado</h4><p>O sorteio aparecer\xE1 aqui.</p></div></section></div>`;
      panel.appendChild(s);
    }
    if (!$r("openRandomizerTool").dataset.bound) {
      $r("openRandomizerTool").dataset.bound = "1";
      $r("openRandomizerTool").onclick = () => {
        $r("studyHome").classList.add("hidden");
        $r("studyCalculator")?.classList.add("hidden");
        $r("studyDocument")?.classList.add("hidden");
        $r("studyRandomizer").classList.remove("hidden");
        refillPeople();
      };
      $r("randomBack").onclick = () => {
        $r("studyRandomizer").classList.add("hidden");
        $r("studyHome").classList.remove("hidden");
      };
      $r("randomDraw").onclick = renderResult;
      $r("randomAgain").onclick = renderResult;
      $r("randomClear").onclick = () => {
        $r("randomPeople").value = "";
        $r("randomItems").value = "";
        $r("randomResult").innerHTML = "<h4>Resultado</h4><p>O sorteio aparecer\xE1 aqui.</p>";
      };
      $r("randomUseGroup").onclick = refillPeople;
      $r("randomMode").onchange = (e) => {
        $r("randomItemsLabel").textContent = e.target.value === "tasks" ? "Tarefas" : e.target.value === "quick" ? "Op\xE7\xF5es" : "Temas";
        $r("randomItems").disabled = e.target.value === "order";
      };
      $r("studyScope")?.addEventListener("change", () => {
        if (!$r("studyRandomizer").classList.contains("hidden")) refillPeople();
      });
    }
    return true;
  }
  ...
  // study-flashcards.js
  var dbF = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  ...
  var quizScore = 0;
  function addCss3() {
    if (document.querySelector('link[href^="study-flashcards.css"]')) return;
    const l = document.createElement("link");
    l.rel = "stylesheet";
  ...
  async function identity4() {
    ...
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await dbF.from("family_members").select("id,family_id,display_name").eq("auth_user_id", user.id).eq("active", true).single();
    if (error) throw error;
    meF = data;
    return data;
  }
  function scopeInfo2() {
    const v = $f("studyScope")?.value || "private";
    return v === "private" ? { conversation_id: null, visibility: "private" } : { conversation_id: v, visibility: "group" };
  }
  ...
  function newDeck() {
    deck = null;
    cards = [{ q: "", a: "" }, { q: "", a: "" }];
    $f("flashTitle").value = "Meus flashcards";
    renderEditor();
    showEditor();
  }
  function showEditor() {
    $f("flashEditorArea").classList.remove("hidden");
    $f("flashStudyArea").classList.add("hidden");
    $f("flashQuizArea").classList.add("hidden");
  }
  function startStudy() {
    ...
    studyIndex = 0;
    showAnswer = false;
    $f("flashEditorArea").classList.add("hidden");
    $f("flashQuizArea").classList.add("hidden");
    $f("flashStudyArea").classList.remove("hidden");
    renderStudy();
  }
  ...
  function startQuiz() {
    ...
    $f("flashEditorArea").classList.add("hidden");
    $f("flashStudyArea").classList.add("hidden");
    $f("flashQuizArea").classList.remove("hidden");
    renderQuiz();
  }
  function renderQuiz() {
    if (quizIndex >= cards.length) {
      $f("flashQuizArea").innerHTML = `<div class="flash-quiz"><h3>Quiz conclu\xEDdo</h3><p>Voc\xEA acertou <strong>${quizScore}</strong> de <strong>${cards.length}</strong>.</p><button id="quizAgain" type="button">Fazer novamente</button><button id="quizEdit" type="button">Voltar aos cart\xF5es</button></div>`;
      $f("quizAgain").onclick = startQuiz;
      $f("quizEdit").onclick = () => {
        mountQuizArea();
        showEditor();
      };
    ...
    document.querySelectorAll("[data-quiz-opt]").forEach((b) => b.onclick = () => {
      ...
      document.querySelectorAll("[data-quiz-opt]").forEach((x) => {
        x.disabled = true;
        if (x.dataset.quizOpt === c.a) x.classList.add("correct");
      });
      if (!correct) b.classList.add("wrong");
      else quizScore++;
      $f("quizFeedback").textContent = correct ? "Acertou!" : "Veja a resposta correta destacada.";
      $f("quizNext").classList.remove("hidden");
    });
    $f("quizNext").onclick = () => {
      quizIndex++;
      renderQuiz();
  ...
  function mountQuizArea() {
    if (!$f("flashQuizArea")) return;
  }
  function mount3() {
    addCss3();
    const grid = document.querySelector(".study-home-grid"), panel = $f("studyPanel");
    if (!grid || !panel) return false;
    if (!$f("openFlashcardsTool")) {
      const b = document.createElement("button");
      b.id = "openFlashcardsTool";
      b.type = "button";
      b.className = "study-tool-card";
      b.style.background = "linear-gradient(145deg,#dff6e8,#e5efff 52%,#eadcff)";
      b.innerHTML = '<span class="tool-symbol">\u{1F5C2}\uFE0F</span><strong>Flashcards + Quiz</strong><small>Crie cart\xF5es de estudo e transforme-os em quiz.</small>';
      grid.appendChild(b);
    }
    if (!$f("studyFlashcards")) {
      const s = document.createElement("div");
      s.id = "studyFlashcards";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="flashBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="flash-shell"><section class="flash-cardbox"><div class="flash-top"><h3>Flashcards</h3><input id="flashTitle" value="Meus flashcards"><button id="flashNew" type="button">Novo</button><button id="flashOpen" type="button">Abrir</button><button id="flashSave" type="button">Salvar</button></div><div id="flashEditorArea"><div id="flashEditor" class="flash-editor"></div><div class="flash-actions"><button id="flashAdd" type="button">\uFF0B Cart\xE3o</button><button id="flashStudyBtn" type="button">Estudar</button><button id="flashQuizBtn" type="button">Criar quiz</button></div></div><div id="flashStudyArea" class="hidden"><div class="flash-study"><button id="flashFlip" class="flash-flip" type="button"></button><div class="flash-nav"><button id="flashPrev" type="button">Anterior</button><button id="flashNext" type="button">Pr\xF3ximo</button><button id="flashEdit" type="button">Editar cart\xF5es</button></div></div></div><div id="flashQuizArea" class="hidden"></div></section></div>`;
      panel.appendChild(s);
    }
    if (!$f("flashModal")) {
      const d = document.createElement("dialog");
      d.id = "flashModal";
      d.className = "flash-modal";
      d.innerHTML = '<div class="flash-modal-inner"><button id="flashModalClose" style="float:right" type="button">\u2715</button><div id="flashModalBody"></div></div>';
      document.body.appendChild(d);
    }
    if (!$f("openFlashcardsTool").dataset.bound) {
      $f("openFlashcardsTool").dataset.bound = "1";
      $f("openFlashcardsTool").onclick = () => {
        $f("studyHome").classList.add("hidden");
        $f("studyCalculator")?.classList.add("hidden");
        $f("studyDocument")?.classList.add("hidden");
        $f("studyRandomizer")?.classList.add("hidden");
        $f("studyFlashcards").classList.remove("hidden");
        renderEditor();
      };
      $f("flashBack").onclick = () => {
        $f("studyFlashcards").classList.add("hidden");
        $f("studyHome").classList.remove("hidden");
      };
      $f("flashNew").onclick = newDeck;
      $f("flashOpen").onclick = openList;
      $f("flashSave").onclick = saveDeck;
      $f("flashAdd").onclick = () => {
        cards.push({ q: "", a: "" });
        renderEditor();
      };
      $f("flashStudyBtn").onclick = startStudy;
      $f("flashQuizBtn").onclick = startQuiz;
      $f("flashFlip").onclick = () => {
      ...
      $f("flashEdit").onclick = showEditor;
      $f("flashModalClose").onclick = () => $f("flashModal").close();
      $f("studyScope")?.addEventListener("change", () => {
        deck = null;
        newDeck();
      });
    }
  ...
  async function identity5() {
    ...
    if (!user) return null;
    const { data } = await db2.from("family_members").select("id").eq("auth_user_id", user.id).eq("active", true).single();
    me2 = data || null;
    return me2;
  }
  function currentScope() {
    const v = $2("studyScope")?.value || "private";
  ...
  async function enhance() {
    const body = $2("flashModalBody");
    if (!body || !body.querySelector("[data-open-flash]")) return;
    const owned = new Set((await listOwned()).map((x) => x.id));
    if (!body.querySelector("#flashDeleteAll")) {
      const h = body.querySelector("h3");
      const bar = document.createElement("div");
      bar.className = "flash-delete-toolbar";
      bar.innerHTML = '<button id="flashDeleteAll" type="button">Apagar meus conjuntos</button>';
      h?.insertAdjacentElement("afterend", bar);
      $2("flashDeleteAll").onclick = deleteAll;
    }
    for (const open of body.querySelectorAll("[data-open-flash]")) {
      const row = open.closest(".flash-list-row"), id = open.dataset.openFlash;
      if (!row || row.querySelector("[data-delete-flash]") || !owned.has(id)) continue;
      const b = document.createElement("button");
      b.type = "button";
      b.dataset.deleteFlash = id;
      b.className = "flash-delete-one";
  ...
  function bind2() {
    const b = $2("flashOpen");
    if (!b || b.dataset.deleteAddon) return false;
    b.dataset.deleteAddon = "1";
  ...
  // study-ideas.js
  var dbI = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  ...
  async function identity6() {
    ...
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await dbI.from("family_members").select("id,family_id,display_name").eq("auth_user_id", user.id).eq("active", true).single();
    if (error) throw error;
    meI = data;
    return data;
  }
  function scopeInfo3() {
    const v = $i("studyScope")?.value || "private";
    return v === "private" ? { conversation_id: null, visibility: "private" } : { conversation_id: v, visibility: "group" };
  }
  function canDeleteNote(n) {
    return !n.owner_id || n.owner_id === meI?.id || scopeInfo3().visibility === "private";
  }
  function normalize(n, i) {
    return { id: n.id || crypto.randomUUID(), text: n.text || "", tone: TONES.includes(n.tone) ? n.tone : "yellow", by: n.by || "", owner_id: n.owner_id || null, x: Number.isFinite(+n.x) ? Math.max(0, +n.x) : 30 + i % 4 * 220, y: Number.isFinite(+n.y) ? Math.max(0, +n.y) : 35 + Math.floor(i / 4) * 190, rotate: Number.isFinite(+n.rotate) ? +n.rotate : [-2, 1, -1, 2, 0][i % 5] };
  }
  function boardDimensions() {
    const c = $i("ideasCanvas");
    return { w: Math.max(c?.clientWidth || 900, 320), h: Math.max(c?.clientHeight || 620, 420) };
  }
  function clampPosition(n) {
    const { w, h } = boardDimensions();
    n.x = Math.max(6, Math.min(w - 190, n.x));
    n.y = Math.max(8, Math.min(h - 150, n.y));
  }
  function render() {
    const c = $i("ideasCanvas");
    if (!c) return;
    ...
    document.querySelectorAll("[data-edit-note]").forEach((x) => x.onclick = (e) => {
      e.stopPropagation();
      editNote(+x.dataset.editNote);
    });
    document.querySelectorAll("[data-drag-note]").forEach((x) => x.onpointerdown = startDrag);
  }
  function startDrag(e) {
    if (e.button !== void 0 && e.button !== 0) return;
    const idx = +e.currentTarget.dataset.dragNote, n = notes[idx], card = e.currentTarget.closest(".idea-note"), canvas = $i("ideasCanvas");
    if (!n || !card || !canvas) return;
    e.preventDefault();
    const r = card.getBoundingClientRect(), cr = canvas.getBoundingClientRect();
    drag = { idx, dx: e.clientX - r.left, dy: e.clientY - r.top, cr, card };
  ...
  function moveDrag(e) {
    ...
    n.x = e.clientX - drag.cr.left - drag.dx + $i("ideasCanvas").scrollLeft;
    n.y = e.clientY - drag.cr.top - drag.dy + $i("ideasCanvas").scrollTop;
    clampPosition(n);
  ...
  function toneButtons() {
    return TONES.map((t) => `<button type="button" class="idea-tone ${t === newTone ? "active" : ""}" data-new-tone="${t}" data-tone="${t}" aria-label="Cor ${t}"></button>`).join("");
  }
  function bindComposer() {
    document.querySelectorAll("[data-new-tone]").forEach((b) => b.onclick = () => {
      newTone = b.dataset.newTone;
      document.querySelectorAll("[data-new-tone]").forEach((x) => x.classList.toggle("active", x.dataset.newTone === newTone));
    });
  }
  ...
  async function load() {
    ...
    notes = [];
    if (channel) {
      dbI.removeChannel(channel);
      channel = null;
    }
    const si = scopeInfo3();
    let q = dbI.from("study_items").select("id,title,content,conversation_id,visibility,updated_at,owner_member_id").eq("item_type", "idea_board").eq("title", "Quadro de Ideias");
    q = si.visibility === "private" ? q.eq("owner_member_id", meI.id).is("conversation_id", null).eq("visibility", "private") : q.eq("conversation_id", si.conversation_id).eq("visibility", "group");
    const { data, error } = await q.order("updated_at", { ascending: false }).limit(1);
    if (error) {
      console.error(error);
      return;
    ...
    board = data?.[0] || null;
    notes = (Array.isArray(board?.content?.notes) ? board.content.notes : []).map(normalize);
    render();
    $i("ideasStatus").textContent = board ? "Salvo \u2713" : "Novo quadro";
    if (board) {
      channel = dbI.channel(`ideas-${board.id}`).on("postgres_changes", { event: "UPDATE", schema: "public", table: "study_items", filter: `id=eq.${board.id}` }, (payload) => {
        if (drag) return;
        notes = (Array.isArray(payload.new?.content?.notes) ? payload.new.content.notes : []).map(normalize);
        render();
        $i("ideasStatus").textContent = "Atualizado em tempo real \u2713";
      }).subscribe();
    }
  }
  function schedule() {
    $i("ideasStatus").textContent = "Salvando\u2026";
  ...
  async function save() {
    ...
    const si = scopeInfo3();
    if (!board) {
      const { data, error } = await dbI.from("study_items").insert({ family_id: meI.family_id, conversation_id: si.conversation_id, owner_member_id: meI.id, item_type: "idea_board", title: "Quadro de Ideias", content: { notes }, visibility: si.visibility, allowed_member_ids: [], updated_by: meI.id }).select("id,title,content,conversation_id,visibility,updated_at,owner_member_id").single();
      if (error) {
        console.error(error);
        $i("ideasStatus").textContent = "Erro ao salvar";
        return;
      }
      board = data;
      await load();
    } else {
      const { error } = await dbI.from("study_items").update({ content: { notes }, updated_by: meI.id }).eq("id", board.id);
      if (error) {
        console.error(error);
        $i("ideasStatus").textContent = "Erro ao salvar";
        return;
  ...
  function mount4() {
    ...
    if (!$i("openIdeasTool")) {
      ...
      b.type = "button";
      b.className = "study-tool-card";
      b.style.background = "linear-gradient(145deg,#fff0bd,#ffddea 48%,#e6dbff)";
      b.innerHTML = '<span class="tool-symbol">\u25A7</span><strong>Quadro de Post-its</strong><small>Estudos, lembretes e ideias em um mural individual ou de grupo.</small>';
      grid.appendChild(b);
    } else {
      $i("openIdeasTool").querySelector("strong").textContent = "Quadro de Post-its";
      const sm = $i("openIdeasTool").querySelector("small");
      if (sm) sm.textContent = "Estudos, lembretes e ideias em um mural individual ou de grupo.";
    }
    if (!$i("studyIdeas")) {
      const s = document.createElement("div");
      s.id = "studyIdeas";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="ideasBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="ideas-shell"><section class="ideas-board"><div class="ideas-top"><div><h3>Quadro de Post-its</h3><span id="ideasStatus" class="ideas-status">Carregando\u2026</span></div><button id="ideasSave" type="button">Salvar agora</button><button id="ideasClearMine" type="button" class="ideas-danger">Apagar meus post-its</button></div><div class="ideas-composer"><div class="idea-color-picker"><span>Cor do post-it</span><div id="ideaTonePicker">${toneButtons()}</div></div><textarea id="ideaComposerText" maxlength="800" placeholder="Ex.: Revisar a mat\xE9ria de Ci\xEAncias; lembrar de levar cartolina; ideia para a apresenta\xE7\xE3o..."></textarea><button id="ideasPinBtn" type="button">Fixar no quadro</button></div><div id="ideasCanvas" class="ideas-canvas"></div></section></div>`;
      panel.appendChild(s);
    } else {
      const old = $i("ideasCanvas");
      if (old && !$i("ideaComposerText")) {
        const top = $i("studyIdeas").querySelector(".ideas-top");
        if (top) {
          top.innerHTML = `<div><h3>Quadro de Post-its</h3><span id="ideasStatus" class="ideas-status">Carregando\u2026</span></div><button id="ideasSave" type="button">Salvar agora</button><button id="ideasClearMine" type="button" class="ideas-danger">Apagar meus post-its</button>`;
          const composer = document.createElement("div");
          composer.className = "ideas-composer";
          composer.innerHTML = `<div class="idea-color-picker"><span>Cor do post-it</span><div id="ideaTonePicker">${toneButtons()}</div></div><textarea id="ideaComposerText" maxlength="800" placeholder="Ex.: Revisar a mat\xE9ria de Ci\xEAncias; lembrar de levar cartolina; ideia para a apresenta\xE7\xE3o..."></textarea><button id="ideasPinBtn" type="button">Fixar no quadro</button>`;
          old.before(composer);
        }
      }
    }
    if (!$i("openIdeasTool").dataset.boundV2) {
      $i("openIdeasTool").dataset.boundV2 = "1";
      $i("openIdeasTool").onclick = () => {
        $i("studyHome").classList.add("hidden");
        ["studyCalculator", "studyDocument", "studyRandomizer", "studyFlashcards", "studyMindmap", "studyPeriodic"].forEach((id) => $i(id)?.classList.add("hidden"));
        $i("studyIdeas").classList.remove("hidden");
        load();
      };
      $i("ideasBack").onclick = () => {
        $i("studyIdeas").classList.add("hidden");
        $i("studyHome").classList.remove("hidden");
      };
      $i("ideasPinBtn").onclick = addFromComposer;
  ...
  // study-mindmap.js
  var dbM = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  ...
  async function identity7() {
    ...
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await dbM.from("family_members").select("id,family_id,display_name").eq("auth_user_id", user.id).eq("active", true).single();
    if (error) throw error;
    meM = data;
    return data;
  }
  function scopeInfo4() {
    const v = $m("studyScope")?.value || "private";
    return v === "private" ? { conversation_id: null, visibility: "private" } : { conversation_id: v, visibility: "group" };
  }
  function lineSvg() {
    return nodes.filter((n) => n.parent).map((n) => {
      ...
      if (!p) return "";
      return `<line x1="${p.x + 75}" y1="${p.y + 28}" x2="${n.x + 75}" y2="${n.y + 28}" stroke="#b9a6d9" stroke-width="3" stroke-linecap="round" opacity=".65"/>`;
    }).join("");
  }
  function render2() {
  ...
  function bindNode(el) {
    ...
    el.ondblclick = () => editNode(el.dataset.node);
    el.onpointerdown = (e) => {
      ...
      const move = (ev) => {
        n.x = Math.max(10, Math.min(1400, ox + ev.clientX - startX));
        n.y = Math.max(10, Math.min(900, oy + ev.clientY - startY));
        render2();
        schedule2();
      };
      const up = () => {
  ...
  function addBranch() {
    const parent = nodes.find((x) => x.id === selectedId) || nodes[0];
    if (!parent) return;
    const idx = nodes.filter((x) => x.parent === parent.id).length;
    nodes.push({ id: crypto.randomUUID(), text: "Nova ideia", parent: parent.id, x: parent.x + 230, y: parent.y + (idx - 1) * 110, tone: ["pink", "blue", "mint", "yellow", "lilac"][nodes.length % 5] });
  ...
  function editNode(id) {
    ...
    if (!n) return;
    const v = prompt("Texto do t\xF3pico:", n.text);
    if (v === null) return;
    n.text = v.trim() || n.text;
    render2();
    schedule2();
  }
  function removeNode() {
    ...
    while (changed2) {
      changed2 = false;
      for (const x of nodes) if (x.parent && remove2.has(x.parent) && !remove2.has(x.id)) {
        remove2.add(x.id);
        changed2 = true;
      }
    }
    nodes = nodes.filter((x) => !remove2.has(x.id));
  ...
  async function load2() {
    ...
    const si = scopeInfo4();
    let q = dbM.from("study_items").select("id,title,content,conversation_id,visibility,updated_at").eq("item_type", "mindmap").eq("title", "Mapa Mental");
    q = si.visibility === "private" ? q.eq("owner_member_id", meM.id).is("conversation_id", null).eq("visibility", "private") : q.eq("conversation_id", si.conversation_id).eq("visibility", "group");
    const { data, error } = await q.order("updated_at", { ascending: false }).limit(1);
    if (error) return;
    mapItem = data?.[0] || null;
    nodes = Array.isArray(mapItem?.content?.nodes) && mapItem.content.nodes.length ? mapItem.content.nodes : [{ id: crypto.randomUUID(), text: "Tema central", parent: null, x: 650, y: 420, tone: "lilac" }];
    selectedId = nodes[0].id;
    render2();
    $m("mindStatus").textContent = mapItem ? "Salvo \u2713" : "Novo mapa";
    if (mapItem) {
      channel2 = dbM.channel(`mind-${mapItem.id}`).on("postgres_changes", { event: "UPDATE", schema: "public", table: "study_items", filter: `id=eq.${mapItem.id}` }, (payload) => {
        if (document.activeElement?.matches?.("[data-node]")) return;
        nodes = payload.new?.content?.nodes || nodes;
        render2();
        $m("mindStatus").textContent = "Atualizado em tempo real \u2713";
      }).subscribe();
    }
  }
  function schedule2() {
    $m("mindStatus").textContent = "Salvando\u2026";
  ...
  function mount5() {
    ...
    if (!$m("openMindmapTool")) {
      const b = document.createElement("button");
      b.id = "openMindmapTool";
      b.type = "button";
      b.className = "study-tool-card";
      b.style.background = "linear-gradient(145deg,#e2efff,#eadfff 48%,#dff5e9)";
      b.innerHTML = '<span class="tool-symbol">\u{1F9E0}</span><strong>Mapa Mental</strong><small>Crie t\xF3picos, ramifica\xE7\xF5es e organize ideias visualmente.</small>';
      grid.appendChild(b);
    }
    if (!$m("studyMindmap")) {
      const s = document.createElement("div");
      s.id = "studyMindmap";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="mindBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="mind-shell"><section class="mind-panel"><div class="mind-top"><h3>Mapa Mental</h3><span id="mindStatus" class="mind-status">Carregando\u2026</span><button id="mindAdd" type="button">\uFF0B Ramifica\xE7\xE3o</button><button id="mindEdit" type="button">Editar t\xF3pico</button><button id="mindRemove" type="button">Excluir t\xF3pico</button><button id="mindSave" type="button">Salvar agora</button></div><div class="mind-hint">Clique para selecionar \u2022 arraste para mover \u2022 clique duas vezes para editar.</div><div id="mindCanvas" class="mind-canvas"></div></section></div>`;
      panel.appendChild(s);
    }
    if (!$m("openMindmapTool").dataset.bound) {
      $m("openMindmapTool").dataset.bound = "1";
      $m("openMindmapTool").onclick = () => {
        $m("studyHome").classList.add("hidden");
        ["studyCalculator", "studyDocument", "studyRandomizer", "studyFlashcards", "studyIdeas"].forEach((id) => $m(id)?.classList.add("hidden"));
        $m("studyMindmap").classList.remove("hidden");
        load2();
      };
      $m("mindBack").onclick = () => {
        $m("studyMindmap").classList.add("hidden");
        $m("studyHome").classList.remove("hidden");
      };
  ...
  // periodic-data.js
  var ELEMENTS = [{ "n": 1, "s": "H", "name": "Hidrog\xEAnio", "mass": "1.008", "group": 1, "period": 1, "cat": "nonmetal", "state": "Gasoso", "use": "combust\xEDveis, produ\xE7\xE3o de am\xF4nia e c\xE9lulas a combust\xEDvel" }, { "n": 2, "s": "He", "name": "H\xE9lio", "mass": "4.0026", "group": 18, "period": 1, "cat": "noble", "state": "Gasoso", "use": "bal\xF5es, criogenia e atmosferas inertes" }, { "n": 3, "s": "Li", "name": "L\xEDtio", "mass": "6.94", "group": 1, "period": 2, "cat": "alkali", "state": "S\xF3lido", "use": "baterias recarreg\xE1veis e ligas leves" }, { "n": 4, "s": "Be", "name": "Ber\xEDlio", "mass": "9.0122", "group": 2, "period": 2, "cat": "alkaline", "state": "S\xF3lido", "use": "ligas aeroespaciais e janelas de raios X" }, { "n": 5, "s": "B", "name": "Boro", "mass": "10.81", "group": 13, "period": 2, "cat": "metalloid", "state": "S\xF3lido", "use": "vidros resistentes, detergentes e materiais semicondutores" }, { "n": 6, "s": "C", "name": "Carbono", "mass": "12.011", "group": 14, "period": 2, "cat": "nonmetal", "state": "S\xF3lido", "use": "base da qu\xEDmica org\xE2nica; grafite, diamante e materiais de carbono" }, { "n": 7, "s": "N", "name": "Nitrog\xEAnio", "mass": "14.007", "group": 15, "period": 2, "cat": "nonmetal", "state": "Gasoso", "use": "fertilizantes, atmosferas inertes e produ\xE7\xE3o de am\xF4nia" }, { "n": 8, "s": "O", "name": "Oxig\xEAnio", "mass": "15.999", "group": 16, "period": 2, "cat": "nonmetal", "state": "Gasoso", "use": "respira\xE7\xE3o, medicina e processos industriais" }, { "n": 9, "s": "F", "name": "Fl\xFAor", "mass": "18.998", "group": 17, "period": 2, "cat": "halogen", "state": "Gasoso", "use": "produ\xE7\xE3o de fluoropol\xEDmeros e compostos fluorados" }, { "n": 10, "s": "Ne", "name": "Ne\xF4nio", "mass": "20.180", "group": 18, "period": 2, "cat": "noble", "state": "Gasoso", "use": "letreiros luminosos e lasers" }, { "n": 11, "s": "Na", "name": "S\xF3dio", "mass": "22.990", "group": 1, "period": 3, "cat": "alkali", "state": "S\xF3lido", "use": "ind\xFAstria qu\xEDmica, ilumina\xE7\xE3o e sais" }, { "n": 12, "s": "Mg", "name": "Magn\xE9sio", "mass": "24.305", "group": 2, "period": 3, "cat": "alkaline", "state": "S\xF3lido", "use": "ligas leves, fogos e suplementos" }, { "n": 13, "s": "Al", "name": "Alum\xEDnio", "mass": "26.982", "group": 13, "period": 3, "cat": "post", "state": "S\xF3lido", "use": "embalagens, estruturas e transporte" }, { "n": 14, "s": "Si", "name": "Sil\xEDcio", "mass": "28.085", "group": 14, "period": 3, "cat": "metalloid", "state": "S\xF3lido", "use": "eletr\xF4nica, chips, vidro e silicones" }, { "n": 15, "s": "P", "name": "F\xF3sforo", "mass": "30.974", "group": 15, "period": 3, "cat": "nonmetal", "state": "S\xF3lido", "use": "fertilizantes, f\xF3sforos e produtos qu\xEDmicos" }, { "n": 16, "s": "S", "name": "Enxofre", "mass": "32.06", "group": 16, "period": 3, "cat": "nonmetal", "state": "S\xF3lido", "use": "fertilizantes, \xE1cido sulf\xFArico e vulcaniza\xE7\xE3o" }, { "n": 17, "s": "Cl", "name": "Cloro", "mass": "35.45", "group": 17, "period": 3, "cat": "halogen", "state": "Gasoso", "use": "tratamento de \xE1gua, desinfec\xE7\xE3o e PVC" }, { "n": 18, "s": "Ar", "name": "Arg\xF4nio", "mass": "39.948", "group": 18, "period": 3, "cat": "noble", "state": "Gasoso", "use": "atmosferas inertes, soldagem e l\xE2mpadas" }, { "n": 19, "s": "K", "name": "Pot\xE1ssio", "mass": "39.098", "group": 1, "period": 4, "cat": "alkali", "state": "S\xF3lido", "use": "fertilizantes e compostos qu\xEDmicos" }, { "n": 20, "s": "Ca", "name": "C\xE1lcio", "mass": "40.078", "group": 2, "period": 4, "cat": "alkaline", "state": "S\xF3lido", "use": "cimento, cal, metalurgia e nutri\xE7\xE3o" }, { "n": 21, "s": "Sc", "name": "Esc\xE2ndio", "mass": "44.956", "group": 3, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 22, "s": "Ti", "name": "Tit\xE2nio", "mass": "47.867", "group": 4, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 23, "s": "V", "name": "Van\xE1dio", "mass": "50.942", "group": 5, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 24, "s": "Cr", "name": "Cromo", "mass": "51.996", "group": 6, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 25, "s": "Mn", "name": "Mangan\xEAs", "mass": "54.938", "group": 7, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 26, "s": "Fe", "name": "Ferro", "mass": "55.845", "group": 8, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "a\xE7o, constru\xE7\xE3o, m\xE1quinas e ferramentas" }, { "n": 27, "s": "Co", "name": "Cobalto", "mass": "58.933", "group": 9, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 28, "s": "Ni", "name": "N\xEDquel", "mass": "58.693", "group": 10, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 29, "s": "Cu", "name": "Cobre", "mass": "63.546", "group": 11, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "fia\xE7\xE3o el\xE9trica, eletr\xF4nica e encanamentos" }, { "n": 30, "s": "Zn", "name": "Zinco", "mass": "65.38", "group": 12, "period": 4, "cat": "transition", "state": "S\xF3lido", "use": "galvaniza\xE7\xE3o, ligas e pilhas" }, { "n": 31, "s": "Ga", "name": "G\xE1lio", "mass": "69.723", "group": 13, "period": 4, "cat": "post", "state": "S\xF3lido", "use": "ligas, eletr\xF4nica e aplica\xE7\xF5es industriais" }, { "n": 32, "s": "Ge", "name": "Germ\xE2nio", "mass": "72.630", "group": 14, "period": 4, "cat": "metalloid", "state": "S\xF3lido", "use": "eletr\xF4nica, ligas e materiais especiais" }, { "n": 33, "s": "As", "name": "Ars\xEAnio", "mass": "74.922", "group": 15, "period": 4, "cat": "metalloid", "state": "S\xF3lido", "use": "eletr\xF4nica, ligas e materiais especiais" }, { "n": 34, "s": "Se", "name": "Sel\xEAnio", "mass": "78.971", "group": 16, "period": 4, "cat": "nonmetal", "state": "S\xF3lido", "use": "compostos qu\xEDmicos e aplica\xE7\xF5es industriais" }, { "n": 35, "s": "Br", "name": "Bromo", "mass": "79.904", "group": 17, "period": 4, "cat": "halogen", "state": "L\xEDquido", "use": "retardantes de chama e fotografia qu\xEDmica" }, { "n": 36, "s": "Kr", "name": "Cript\xF4nio", "mass": "83.798", "group": 18, "period": 4, "cat": "noble", "state": "Gasoso", "use": "ilumina\xE7\xE3o, atmosferas inertes e pesquisa" }, { "n": 37, "s": "Rb", "name": "Rub\xEDdio", "mass": "85.468", "group": 1, "period": 5, "cat": "alkali", "state": "S\xF3lido", "use": "compostos qu\xEDmicos e aplica\xE7\xF5es industriais" }, { "n": 38, "s": "Sr", "name": "Estr\xF4ncio", "mass": "87.62", "group": 2, "period": 5, "cat": "alkaline", "state": "S\xF3lido", "use": "compostos qu\xEDmicos e aplica\xE7\xF5es industriais" }, { "n": 39, "s": "Y", "name": "\xCDtrio", "mass": "88.906", "group": 3, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 40, "s": "Zr", "name": "Zirc\xF4nio", "mass": "91.224", "group": 4, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 41, "s": "Nb", "name": "Ni\xF3bio", "mass": "92.906", "group": 5, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 42, "s": "Mo", "name": "Molibd\xEAnio", "mass": "95.95", "group": 6, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 43, "s": "Tc", "name": "Tecn\xE9cio", "mass": "[98]", "group": 7, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 44, "s": "Ru", "name": "Rut\xEAnio", "mass": "101.07", "group": 8, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 45, "s": "Rh", "name": "R\xF3dio", "mass": "102.91", "group": 9, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 46, "s": "Pd", "name": "Pal\xE1dio", "mass": "106.42", "group": 10, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 47, "s": "Ag", "name": "Prata", "mass": "107.87", "group": 11, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "joias, eletr\xF4nica, espelhos e fotografia" }, { "n": 48, "s": "Cd", "name": "C\xE1dmio", "mass": "112.41", "group": 12, "period": 5, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 49, "s": "In", "name": "\xCDndio", "mass": "114.82", "group": 13, "period": 5, "cat": "post", "state": "S\xF3lido", "use": "ligas, eletr\xF4nica e aplica\xE7\xF5es industriais" }, { "n": 50, "s": "Sn", "name": "Estanho", "mass": "118.71", "group": 14, "period": 5, "cat": "post", "state": "S\xF3lido", "use": "ligas, eletr\xF4nica e aplica\xE7\xF5es industriais" }, { "n": 51, "s": "Sb", "name": "Antim\xF4nio", "mass": "121.76", "group": 15, "period": 5, "cat": "metalloid", "state": "S\xF3lido", "use": "eletr\xF4nica, ligas e materiais especiais" }, { "n": 52, "s": "Te", "name": "Tel\xFArio", "mass": "127.60", "group": 16, "period": 5, "cat": "metalloid", "state": "S\xF3lido", "use": "eletr\xF4nica, ligas e materiais especiais" }, { "n": 53, "s": "I", "name": "Iodo", "mass": "126.90", "group": 17, "period": 5, "cat": "halogen", "state": "S\xF3lido", "use": "antiss\xE9pticos, nutri\xE7\xE3o e aplica\xE7\xF5es m\xE9dicas" }, { "n": 54, "s": "Xe", "name": "Xen\xF4nio", "mass": "131.29", "group": 18, "period": 5, "cat": "noble", "state": "Gasoso", "use": "l\xE2mpadas especiais, flashes e propuls\xE3o i\xF4nica" }, { "n": 55, "s": "Cs", "name": "C\xE9sio", "mass": "132.91", "group": 1, "period": 6, "cat": "alkali", "state": "S\xF3lido", "use": "compostos qu\xEDmicos e aplica\xE7\xF5es industriais" }, { "n": 56, "s": "Ba", "name": "B\xE1rio", "mass": "137.33", "group": 2, "period": 6, "cat": "alkaline", "state": "S\xF3lido", "use": "compostos qu\xEDmicos e aplica\xE7\xF5es industriais" }, { "n": 57, "s": "La", "name": "Lant\xE2nio", "mass": "138.91", "group": 3, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "lentes, catalisadores e ligas" }, { "n": 58, "s": "Ce", "name": "C\xE9rio", "mass": "140.12", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "catalisadores, polimento de vidro e ligas" }, { "n": 59, "s": "Pr", "name": "Praseod\xEDmio", "mass": "140.91", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s, \xF3ptica, eletr\xF4nica e materiais especiais" }, { "n": 60, "s": "Nd", "name": "Neod\xEDmio", "mass": "144.24", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s permanentes e lasers" }, { "n": 61, "s": "Pm", "name": "Prom\xE9cio", "mass": "[145]", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s, \xF3ptica, eletr\xF4nica e materiais especiais" }, { "n": 62, "s": "Sm", "name": "Sam\xE1rio", "mass": "150.36", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s, reatores e materiais especiais" }, { "n": 63, "s": "Eu", "name": "Eur\xF3pio", "mass": "151.96", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "f\xF3sforos vermelhos e aplica\xE7\xF5es nucleares" }, { "n": 64, "s": "Gd", "name": "Gadol\xEDnio", "mass": "157.25", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "contraste em resson\xE2ncia magn\xE9tica e materiais magn\xE9ticos" }, { "n": 65, "s": "Tb", "name": "T\xE9rbio", "mass": "158.93", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s, \xF3ptica, eletr\xF4nica e materiais especiais" }, { "n": 66, "s": "Dy", "name": "Dispr\xF3sio", "mass": "162.50", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s de alta pot\xEAncia" }, { "n": 67, "s": "Ho", "name": "H\xF3lmio", "mass": "164.93", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s, \xF3ptica, eletr\xF4nica e materiais especiais" }, { "n": 68, "s": "Er", "name": "\xC9rbio", "mass": "167.26", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "fibras \xF3pticas e lasers" }, { "n": 69, "s": "Tm", "name": "T\xFAlio", "mass": "168.93", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "\xEDm\xE3s, \xF3ptica, eletr\xF4nica e materiais especiais" }, { "n": 70, "s": "Yb", "name": "It\xE9rbio", "mass": "173.05", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "lasers, ligas e pesquisa" }, { "n": 71, "s": "Lu", "name": "Lut\xE9cio", "mass": "174.97", "group": null, "period": 6, "cat": "lanthanide", "state": "S\xF3lido", "use": "catalisadores e pesquisa" }, { "n": 72, "s": "Hf", "name": "H\xE1fnio", "mass": "178.49", "group": 4, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 73, "s": "Ta", "name": "T\xE2ntalo", "mass": "180.95", "group": 5, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 74, "s": "W", "name": "Tungst\xEAnio", "mass": "183.84", "group": 6, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "ferramentas de alta temperatura e filamentos" }, { "n": 75, "s": "Re", "name": "R\xEAnio", "mass": "186.21", "group": 7, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 76, "s": "Os", "name": "\xD3smio", "mass": "190.23", "group": 8, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 77, "s": "Ir", "name": "Ir\xEDdio", "mass": "192.22", "group": 9, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "ligas met\xE1licas, catalisadores e aplica\xE7\xF5es industriais" }, { "n": 78, "s": "Pt", "name": "Platina", "mass": "195.08", "group": 10, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "catalisadores, joias e equipamentos de laborat\xF3rio" }, { "n": 79, "s": "Au", "name": "Ouro", "mass": "196.97", "group": 11, "period": 6, "cat": "transition", "state": "S\xF3lido", "use": "joias, eletr\xF4nica e odontologia" }, { "n": 80, "s": "Hg", "name": "Merc\xFArio", "mass": "200.59", "group": 12, "period": 6, "cat": "transition", "state": "L\xEDquido", "use": "instrumentos hist\xF3ricos, l\xE2mpadas e processos qu\xEDmicos" }, { "n": 81, "s": "Tl", "name": "T\xE1lio", "mass": "204.38", "group": 13, "period": 6, "cat": "post", "state": "S\xF3lido", "use": "ligas, eletr\xF4nica e aplica\xE7\xF5es industriais" }, { "n": 82, "s": "Pb", "name": "Chumbo", "mass": "207.2", "group": 14, "period": 6, "cat": "post", "state": "S\xF3lido", "use": "baterias, prote\xE7\xE3o contra radia\xE7\xE3o e ligas" }, { "n": 83, "s": "Bi", "name": "Bismuto", "mass": "208.98", "group": 15, "period": 6, "cat": "post", "state": "S\xF3lido", "use": "ligas de baixo ponto de fus\xE3o e medicamentos espec\xEDficos" }, { "n": 84, "s": "Po", "name": "Pol\xF4nio", "mass": "[209]", "group": 16, "period": 6, "cat": "post", "state": "S\xF3lido", "use": "ligas, eletr\xF4nica e aplica\xE7\xF5es industriais" }, { "n": 85, "s": "At", "name": "Astato", "mass": "[210]", "group": 17, "period": 6, "cat": "halogen", "state": "S\xF3lido", "use": "compostos qu\xEDmicos, s\xEDntese e aplica\xE7\xF5es industriais" }, { "n": 86, "s": "Rn", "name": "Rad\xF4nio", "mass": "[222]", "group": 18, "period": 6, "cat": "noble", "state": "Gasoso", "use": "pesquisa e estudos geol\xF3gicos; \xE9 radioativo" }, { "n": 87, "s": "Fr", "name": "Fr\xE2ncio", "mass": "[223]", "group": 1, "period": 7, "cat": "alkali", "state": "S\xF3lido", "use": "compostos qu\xEDmicos e aplica\xE7\xF5es industriais" }, { "n": 88, "s": "Ra", "name": "R\xE1dio", "mass": "[226]", "group": 2, "period": 7, "cat": "alkaline", "state": "S\xF3lido", "use": "pesquisa e aplica\xE7\xF5es hist\xF3ricas em radiologia" }, { "n": 89, "s": "Ac", "name": "Act\xEDnio", "mass": "[227]", "group": 3, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa nuclear e cient\xEDfica" }, { "n": 90, "s": "Th", "name": "T\xF3rio", "mass": "232.04", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "pesquisa nuclear e materiais especiais" }, { "n": 91, "s": "Pa", "name": "Protact\xEDnio", "mass": "231.04", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa nuclear e cient\xEDfica" }, { "n": 92, "s": "U", "name": "Ur\xE2nio", "mass": "238.03", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "combust\xEDvel nuclear e pesquisa" }, { "n": 93, "s": "Np", "name": "Net\xFAnio", "mass": "[237]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 94, "s": "Pu", "name": "Plut\xF4nio", "mass": "[244]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "combust\xEDvel nuclear, fontes de energia e pesquisa" }, { "n": 95, "s": "Am", "name": "Amer\xEDcio", "mass": "[243]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "detectores de fuma\xE7a e pesquisa" }, { "n": 96, "s": "Cm", "name": "C\xFArio", "mass": "[247]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 97, "s": "Bk", "name": "Berqu\xE9lio", "mass": "[247]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 98, "s": "Cf", "name": "Calif\xF3rnio", "mass": "[251]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "fontes de n\xEAutrons e pesquisa" }, { "n": 99, "s": "Es", "name": "Einst\xEAnio", "mass": "[252]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 100, "s": "Fm", "name": "F\xE9rmio", "mass": "[257]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 101, "s": "Md", "name": "Mendel\xE9vio", "mass": "[258]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 102, "s": "No", "name": "Nob\xE9lio", "mass": "[259]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 103, "s": "Lr", "name": "Laur\xEAncio", "mass": "[266]", "group": null, "period": 7, "cat": "actinide", "state": "S\xF3lido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 104, "s": "Rf", "name": "Rutherf\xF3rdio", "mass": "[267]", "group": 4, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 105, "s": "Db", "name": "D\xFAbnio", "mass": "[268]", "group": 5, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 106, "s": "Sg", "name": "Seab\xF3rgio", "mass": "[269]", "group": 6, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 107, "s": "Bh", "name": "B\xF3hrio", "mass": "[270]", "group": 7, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 108, "s": "Hs", "name": "H\xE1ssio", "mass": "[277]", "group": 8, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 109, "s": "Mt", "name": "Meitn\xE9rio", "mass": "[278]", "group": 9, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 110, "s": "Ds", "name": "Darmst\xE1dio", "mass": "[281]", "group": 10, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 111, "s": "Rg", "name": "Roentg\xEAnio", "mass": "[282]", "group": 11, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 112, "s": "Cn", "name": "Copern\xEDcio", "mass": "[285]", "group": 12, "period": 7, "cat": "transition", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 113, "s": "Nh", "name": "Nih\xF4nio", "mass": "[286]", "group": 13, "period": 7, "cat": "post", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 114, "s": "Fl", "name": "Fler\xF3vio", "mass": "[289]", "group": 14, "period": 7, "cat": "post", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 115, "s": "Mc", "name": "Mosc\xF3vio", "mass": "[290]", "group": 15, "period": 7, "cat": "post", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 116, "s": "Lv", "name": "Liverm\xF3rio", "mass": "[293]", "group": 16, "period": 7, "cat": "post", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 117, "s": "Ts", "name": "Tenessino", "mass": "[294]", "group": 17, "period": 7, "cat": "halogen", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }, { "n": 118, "s": "Og", "name": "Oganess\xF4nio", "mass": "[294]", "group": 18, "period": 7, "cat": "noble", "state": "Desconhecido", "use": "principalmente pesquisa cient\xEDfica e nuclear" }];
  ...
  function match(e) {
    const q = ($p("periodicSearch")?.value || "").trim().toLowerCase(), cat = $p("periodicFilter")?.value || "all";
  ...
  function render3() {
    ...
    $p("periodicGrid").innerHTML = main.map(tile).join("");
    $p("lanSeries").innerHTML = '<div class="series-label">Lantan\xEDdeos</div>' + lan.map(seriesTile).join("");
    $p("actSeries").innerHTML = '<div class="series-label">Actin\xEDdeos</div>' + act.map(seriesTile).join("");
    document.querySelectorAll("[data-element]").forEach((b) => b.onclick = (e) => {
      if (e.shiftKey) toggleCompare(+b.dataset.element);
      else openElement(+b.dataset.element);
    });
    paintCompare();
  }
  function openElement(n) {
    ...
    if (!e) return;
    let d = $p("periodicDetail");
    if (!d) {
      d = document.createElement("dialog");
      d.id = "periodicDetail";
      d.className = "periodic-detail";
      d.innerHTML = '<div class="periodic-detail-inner"><button id="periodicClose" style="float:right" type="button">\u2715</button><div id="periodicDetailBody"></div></div>';
      document.body.appendChild(d);
  ...
  function toggleCompare(n) {
    if (compare.includes(n)) compare = compare.filter((x) => x !== n);
    else {
  ...
  function showCompare() {
    const [a, b] = compare.map((n) => ELEMENTS.find((x) => x.n === n));
    if (!a || !b) return;
    let d = $p("periodicDetail");
    if (!d) {
      openElement(a.n);
  ...
  function mount6() {
    ...
    if (!$p("openPeriodicTool")) {
      const b = document.createElement("button");
      b.id = "openPeriodicTool";
      b.type = "button";
      b.className = "study-tool-card";
      b.style.background = "linear-gradient(145deg,#dff4ff,#e6ddff 48%,#dff5e8)";
      b.innerHTML = '<span class="tool-symbol">\u269B\uFE0F</span><strong>Tabela Peri\xF3dica</strong><small>118 elementos com busca, filtros, detalhes e compara\xE7\xE3o.</small>';
      grid.appendChild(b);
    }
    if (!$p("studyPeriodic")) {
      const s = document.createElement("div");
      s.id = "studyPeriodic";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="periodicBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="periodic-shell"><section class="periodic-panel"><div class="periodic-top"><h3>Tabela Peri\xF3dica Interativa</h3><input id="periodicSearch" type="search" placeholder="Buscar nome, s\xEDmbolo ou n\xFAmero"><select id="periodicFilter"><option value="all">Todas as categorias</option>${Object.entries(CAT).map(([k, v]) => `<option value="${k}">${v}</option>`).join("")}</select></div><div id="periodicCompare" class="compare-bar"></div><div class="periodic-legend">${Object.entries(CAT).filter(([k]) => k !== "unknown").map(([k, v]) => `<span class="cat-${k}">${v}</span>`).join("")}</div><div class="periodic-table-wrap"><div id="periodicGrid" class="periodic-grid"></div><div id="lanSeries" class="periodic-series"></div><div id="actSeries" class="periodic-series"></div></div><small>Toque em um elemento para abrir os detalhes. Use \u201CComparar este elemento\u201D para comparar dois.</small></section></div>`;
      panel.appendChild(s);
    }
    if (!$p("openPeriodicTool").dataset.bound) {
      $p("openPeriodicTool").dataset.bound = "1";
      $p("openPeriodicTool").onclick = () => {
        $p("studyHome").classList.add("hidden");
        ["studyCalculator", "studyDocument", "studyRandomizer", "studyFlashcards", "studyIdeas", "studyMindmap"].forEach((id) => $p(id)?.classList.add("hidden"));
        $p("studyPeriodic").classList.remove("hidden");
        render3();
      };
      $p("periodicBack").onclick = () => {
        $p("studyPeriodic").classList.add("hidden");
        $p("studyHome").classList.remove("hidden");
      };
      $p("periodicSearch").oninput = render3;
      $p("periodicFilter").onchange = render3;
    }
    return true;
  }
  function wait7() {
  ...
  async function identity8() {
    ...
    if (!user) return null;
    const { data } = await db3.from("family_members").select("id").eq("auth_user_id", user.id).eq("active", true).single();
    me3 = data || null;
    return me3;
  }
  function scope3() {
  ...
  function mount7() {
    const grid = document.querySelector(".study-home-grid"), panel = $3("studyPanel");
    if (!grid || !panel) return false;
    if (!$3("openMaterialsTool")) {
      const b = document.createElement("button");
      b.id = "openMaterialsTool";
      b.type = "button";
      b.className = "study-tool-card";
      b.innerHTML = '<span class="tool-symbol">\u25A4</span><strong>Meus materiais</strong><small>Veja e exclua tudo o que voc\xEA salvou neste espa\xE7o.</small>';
      grid.appendChild(b);
    }
    if (!$3("studyMaterials")) {
      const s = document.createElement("div");
      s.id = "studyMaterials";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="materialsBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="materials-shell"><div class="materials-head"><div><h3>Meus materiais</h3><p class="muted">Somente os materiais criados por voc\xEA aparecem aqui.</p></div><button id="materialsDeleteAll" type="button">Apagar tudo</button></div><div id="materialsList"></div></div>`;
      panel.appendChild(s);
    }
    if (!$3("openMaterialsTool").dataset.bound) {
      $3("openMaterialsTool").dataset.bound = "1";
      $3("openMaterialsTool").onclick = () => {
        $3("studyHome").classList.add("hidden");
        document.querySelectorAll(".study-workspace").forEach((x) => x.classList.add("hidden"));
        $3("studyMaterials").classList.remove("hidden");
        render4();
      };
      $3("materialsBack").onclick = () => {
        $3("studyMaterials").classList.add("hidden");
        $3("studyHome").classList.remove("hidden");
      };
      $3("materialsDeleteAll").onclick = removeAll;
      $3("studyScope")?.addEventListener("change", () => {
        if (!$3("studyMaterials").classList.contains("hidden")) render4();
      });
    }
    return true;
  }
  ...
  // study-timer.js
  var dbT = createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_KEY, { auth: { persistSession: true, autoRefreshToken: true } });
  ...
  async function identity9() {
    ...
    if (!user) throw new Error("Sem sess\xE3o");
    const { data, error } = await dbT.from("family_members").select("id,family_id,display_name").eq("auth_user_id", user.id).eq("active", true).single();
    if (error) throw error;
    meT = data;
    return data;
  }
  ...
  function fresh(mode = "response") {
    const d = mode === "study" ? 25 * 60 * 1e3 : mode === "response" ? 60 * 1e3 : 0;
    return { mode, duration_ms: d, remaining_ms: d, elapsed_ms: 0, running: false, started_at: null, target_member_id: null, target_name: "", updated_by_name: meT?.display_name || "" };
  }
  function nowValue() {
    if (!stateT) return 0;
    if (stateT.mode === "stopwatch") {
      const base2 = Number(stateT.elapsed_ms) || 0;
      return stateT.running && stateT.started_at ? base2 + Math.max(0, Date.now() - new Date(stateT.started_at).getTime()) : base2;
    }
    const base = Number(stateT.remaining_ms) || 0;
    return stateT.running && stateT.started_at ? Math.max(0, base - Math.max(0, Date.now() - new Date(stateT.started_at).getTime())) : base;
  }
  function format(ms) {
    ms = Math.max(0, Math.round(ms / 1e3));
    const h = Math.floor(ms / 3600), m = Math.floor(ms % 3600 / 60), s = ms % 60;
    return h ? `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}` : `${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}`;
  }
  function durationInputs(ms) {
    const sec = Math.max(0, Math.round(ms / 1e3));
    if ($t("timerMinutes")) $t("timerMinutes").value = Math.floor(sec / 60);
    if ($t("timerSeconds")) $t("timerSeconds").value = sec % 60;
  }
  function durationFromInputs() {
    const m = Math.max(0, Number($t("timerMinutes")?.value) || 0), s = Math.max(0, Math.min(59, Number($t("timerSeconds")?.value) || 0));
    return (m * 60 + s) * 1e3;
  }
  function modeLabel() {
    return stateT?.mode === "response" ? "Tempo para responder" : stateT?.mode === "study" ? "Modo Estudo" : "Cron\xF4metro livre";
  }
  function targetLabel() {
    if (stateT?.mode !== "response") return stateT?.mode === "study" ? "Foco no estudo" : "Tempo decorrido";
    return stateT.target_name ? `Vez de ${stateT.target_name}` : "Escolha quem vai responder";
  }
  function render5() {
    if (!$t("timerTime") || !stateT) return;
    const value = nowValue();
    $t("timerTime").textContent = format(value);
    $t("timerTarget").textContent = targetLabel();
    $t("timerModeName").textContent = modeLabel();
    document.querySelectorAll("[data-timer-mode]").forEach((b) => b.classList.toggle("active", b.dataset.timerMode === stateT.mode));
    $t("timerDurationPanel")?.classList.toggle("hidden", stateT.mode === "stopwatch");
    $t("timerTargetWrap")?.classList.toggle("hidden", stateT.mode !== "response");
    const ring = $t("timerRing");
    if (ring) {
      let p = 0;
      if (stateT.mode !== "stopwatch" && stateT.duration_ms > 0) p = Math.max(0, Math.min(1, 1 - value / stateT.duration_ms));
      ring.style.setProperty("--progress", `${p * 360}deg`);
    }
    $t("timerStatus").textContent = stateT.running ? "Rodando em tempo real" : itemT ? "Pausado \u2022 salvo" : "Pronto para come\xE7ar";
    if (stateT.mode !== "stopwatch" && value <= 0 && stateT.running) finishTimer();
  }
  function beep() {
    try {
      ...
      g.gain.exponentialRampToValueAtTime(1e-3, a.currentTime + 0.45);
      o.connect(g);
  ...
  function finishTimer() {
    const key = stateT?.started_at || "done";
    ...
    if (itemT?.owner_member_id === meT?.id) {
      const current = { ...stateT, running: false, remaining_ms: 0, started_at: null };
      stateT = current;
      persist(false);
    }
  }
  async function loadMembers() {
    ...
    if (s.private) {
      ...
      const { data: cms } = await dbT.from("conversation_members").select("member_id").eq("conversation_id", s.id);
      const ids = (cms || []).map((x) => x.member_id);
      if (ids.length) {
        const { data } = await dbT.from("family_members").select("id,display_name").in("id", ids).eq("active", true).order("display_name");
        membersT = data || [];
      } else membersT = [];
    }
    const sel = $t("timerTargetSelect");
    if (sel) {
      sel.innerHTML = '<option value="">Escolha uma pessoa</option>' + membersT.map((m) => `<option value="${m.id}">${m.display_name}</option>`).join("");
      if (stateT?.target_member_id) sel.value = stateT.target_member_id;
    }
  }
  async function load3() {
    ...
    itemT = data?.[0] || null;
    stateT = itemT?.content?.timer || fresh("response");
    if (!stateT.mode) stateT = fresh("response");
    durationInputs(stateT.duration_ms || 6e4);
    await loadMembers();
    render5();
    if (itemT) {
      channelT = dbT.channel(`timer-${itemT.id}`).on("postgres_changes", { event: "UPDATE", schema: "public", table: "study_items", filter: `id=eq.${itemT.id}` }, (p) => {
        const next = p.new?.content?.timer;
        if (!next) return;
        stateT = next;
        durationInputs(stateT.duration_ms || 0);
        loadMembers().then(render5);
        finishedKey = "";
      }).subscribe();
    }
    startTick();
  ...
  function setMode(mode) {
    ...
    if (mode === "response" && membersT.length === 1) {
      ...
      stateT.target_name = membersT[0].display_name;
    }
    durationInputs(stateT.duration_ms);
    finishedKey = "";
    render5();
    persist(false);
  }
  function setDuration(ms) {
    ...
    durationInputs(ms);
    document.querySelectorAll("[data-timer-preset]").forEach((b) => b.classList.toggle("active", Number(b.dataset.timerPreset) === ms));
    render5();
    persist(false);
  }
  function customDuration() {
    if (!stateT || stateT.mode === "stopwatch" || stateT.running) return;
    const ms = durationFromInputs();
    if (ms < 1e3) return;
    stateT.duration_ms = ms;
    stateT.remaining_ms = ms;
    render5();
  ...
  function start() {
    if (!stateT) return;
    if (stateT.running) return;
    if (stateT.mode !== "stopwatch" && nowValue() <= 0) {
  ...
  function pause() {
    if (!stateT?.running) return;
    const v = nowValue();
    if (stateT.mode === "stopwatch") stateT.elapsed_ms = v;
    else stateT.remaining_ms = v;
    stateT.running = false;
    stateT.started_at = null;
    persist(false);
    render5();
  }
  function reset() {
    ...
    stateT.started_at = null;
    finishedKey = "";
    if (stateT.mode === "stopwatch") stateT.elapsed_ms = 0;
    else stateT.remaining_ms = stateT.duration_ms;
    persist(false);
    render5();
  }
  function targetChanged() {
    const id = $t("timerTargetSelect").value, m = membersT.find((x) => x.id === id);
    stateT.target_member_id = id || null;
    stateT.target_name = m?.display_name || "";
    persist(false);
    render5();
  }
  function startTick() {
  ...
  function mount8() {
    ...
    if (!$t("openTimerTool")) {
      ...
      b.type = "button";
      b.className = "study-tool-card";
      b.style.background = "linear-gradient(145deg,#e1f3ff,#efe4ff 52%,#ffe4ee)";
      b.innerHTML = '<span class="tool-symbol">\u25F7</span><strong>Cron\xF4metro & Estudo</strong><small>Marque tempo de estudo ou d\xEA tempo para algu\xE9m responder.</small>';
      grid.appendChild(b);
    }
    if (!$t("studyTimer")) {
      const s = document.createElement("div");
      s.id = "studyTimer";
      s.className = "study-workspace hidden";
      s.innerHTML = `<button id="timerBack" class="study-back" type="button">\u2190 Mesa de Estudos</button><div class="timer-shell"><section class="timer-card"><div class="timer-head"><div><h3>Cron\xF4metro & Modo Estudo</h3><p class="muted">Use sozinho ou sincronizado com o grupo.</p></div><span id="timerStatus" class="timer-status">Carregando\u2026</span></div><div class="timer-controls-grid"><div class="timer-panel"><label>Modo</label><div class="timer-mode-row"><button data-timer-mode="response" type="button">Responder</button><button data-timer-mode="study" type="button">Estudo</button><button data-timer-mode="stopwatch" type="button">Cron\xF4metro</button></div><div id="timerTargetWrap" class="timer-answer-card"><strong>Quem vai responder?</strong><select id="timerTargetSelect"></select></div><div id="timerDurationPanel"><label>Tempo</label><div class="timer-preset-row">${PRESETS.map((s2) => `<button data-timer-preset="${s2 * 1e3}" type="button">${s2 < 60 ? `${s2}s` : s2 === 1500 ? "25 min" : `${s2 / 60} min`}</button>`).join("")}</div><div class="timer-custom"><label>Minutos<input id="timerMinutes" type="number" min="0" max="180" value="1"></label><label>Segundos<input id="timerSeconds" type="number" min="0" max="59" value="0"></label></div></div></div><div class="timer-display-wrap"><div id="timerRing" class="timer-ring"><div class="timer-display"><small id="timerModeName">Tempo para responder</small><div id="timerTime" class="timer-time">01:00</div><div id="timerTarget" class="timer-target"></div><div class="timer-sub">Sincronizado neste espa\xE7o</div></div></div></div></div><div class="timer-action-row"><button id="timerStart" class="timer-start" type="button">Iniciar</button><button id="timerPause" class="timer-pause" type="button">Pausar</button><button id="timerReset" class="timer-reset" type="button">Zerar</button></div></section></div>`;
      panel.appendChild(s);
    }
    if (!$t("openTimerTool").dataset.bound) {
      $t("openTimerTool").dataset.bound = "1";
      $t("openTimerTool").onclick = () => {
        $t("studyHome").classList.add("hidden");
        document.querySelectorAll(".study-workspace").forEach((x) => x.classList.add("hidden"));
        $t("studyTimer").classList.remove("hidden");
        load3();
      };
      $t("timerBack").onclick = () => {
        $t("studyTimer").classList.add("hidden");
        $t("studyHome").classList.remove("hidden");
      };
      document.querySelectorAll("[data-timer-mode]").forEach((b) => b.onclick = () => setMode(b.dataset.timerMode));
      document.querySelectorAll("[data-timer-preset]").forEach((b) => b.onclick = () => setDuration(Number(b.dataset.timerPreset)));
      $t("timerMinutes").onchange = customDuration;
      $t("timerSeconds").onchange = customDuration;
      $t("timerTargetSelect").onchange = targetChanged;
      $t("timerStart").onclick = start;
      $t("timerPause").onclick = pause;
      $t("timerReset").onclick = reset;
      $t("studyScope")?.addEventListener("change", () => {
        if (!$t("studyTimer").classList.contains("hidden")) load3();
      });
    }
    return true;
  }
  ...
  dbT.auth.onAuthStateChange((_e, s) => {
    if (!s) {
      ...
      itemT = null;
      stateT = null;
      if (channelT) dbT.removeChannel(channelT);
      clearInterval(tickT);
    }
  });
})();
