...
function toast(text){const t=$('toast');if(!t){alert(text);return}t.textContent=text;t.classList.remove('hidden');clearTimeout(t._nv3);t._nv3=setTimeout(()=>t.classList.add('hidden'),3800)}
...
async function memberId(){const t=token(),sub=jwtSub(t);if(!t||!sub)throw new Error('Entre novamente no Cantinho antes de ativar as notificações.');const r=await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/family_members?select=id&auth_user_id=eq.${encodeURIComponent(sub)}&active=eq.true&limit=1`,{headers:{apikey:CONFIG.SUPABASE_KEY,Authorization:`Bearer ${t}`},cache:'no-store'}),d=await r.json();if(!r.ok||!d?.[0]?.id)throw new Error('Não foi possível identificar este perfil.');return d[0].id}
async function waitReady(ms=7000){return Promise.race([navigator.serviceWorker.ready,new Promise((_,rej)=>setTimeout(()=>rej(new Error('O serviço de notificações demorou para iniciar. Recarregue o Cantinho e tente novamente.')),ms))])}
async function registerSubscription(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('./sw.js?v=32',{scope:'./'})
  await waitReady()
  ...
  const j=sub.toJSON(),mid=await memberId(),t=token(),body={member_id:mid,platform:'web',endpoint:j.endpoint,p256dh:j.keys?.p256dh||'',auth_key:j.keys?.auth||'',device_name:/Android|iPhone|iPad|Mobile/i.test(navigator.userAgent)?'Celular':'Computador',enabled:true,child_message_alarm:true,updated_at:new Date().toISOString()}
...
async function activate(e){
  ...
  if(!('Notification'in window)){toast('Este navegador não oferece notificações para sites.');return}
  ...
  try{
    ...
    localStorage.setItem('isa-notifications-enabled','1');$('notifyBanner')?.classList.add('hidden')
  ...
  }catch(err){console.error('Notificações:',err);toast(err?.message||'Não foi possível ativar as notificações.');b.textContent=Notification.permission==='denied'?'Bloqueadas':'Ativar'}
...
function wire(){const b=$('enableNotificationsBtn');if(!b||b.dataset.notifyV3)return;b.dataset.notifyV3='1';b.addEventListener('click',activate,true);reflectExisting()}
wire();const main=$('mainView');if(main){const o=new MutationObserver(()=>wire());o.observe(main,{attributes:true,attributeFilter:['class']})}
