...
import { CONFIG } from './config.js?v=20260910-profile-theme-color-live'
...
#socialPanel,#familySocialOverlay{--isa-profile-accent:#c7b5ee;--isa-profile-soft:rgba(199,181,238,.18);--isa-profile-veil:rgba(199,181,238,.10)}
#socialPanel.isa-profile-theme-live,#familySocialOverlay.isa-profile-theme-live{background:linear-gradient(135deg,color-mix(in srgb,var(--isa-profile-accent) 16%,#fff8fc),#f8f4ff 54%,color-mix(in srgb,var(--isa-profile-accent) 10%,#eef8ff))!important;transition:background .28s ease}
...
#socialPanel .social-side.isa-profile-themed,#familySocialOverlay .social-side.isa-profile-themed,
...
#socialPanel.isa-profile-theme-live .social-right,#familySocialOverlay.isa-profile-theme-live .social-right{background:linear-gradient(145deg,rgba(255,255,255,.96),color-mix(in srgb,var(--isa-profile-accent) 12%,#fff))!important;border-color:color-mix(in srgb,var(--isa-profile-accent) 38%,#fff)!important;box-shadow:0 16px 34px color-mix(in srgb,var(--isa-profile-accent) 14%,transparent)!important}
#socialPanel .social-side .social-pill,#familySocialOverlay .social-side .social-pill,
#socialPanel.isa-profile-theme-live .social-pill,#familySocialOverlay.isa-profile-theme-live .social-pill{background:linear-gradient(135deg,#fff,var(--isa-profile-soft))!important;border-color:color-mix(in srgb,var(--isa-profile-accent) 28%,#eee)!important}
#socialPanel .social-btn.primary,#familySocialOverlay .social-btn.primary,
#socialPanel .spp-primary,#familySocialOverlay .spp-primary{background:linear-gradient(135deg,var(--isa-profile-accent),color-mix(in srgb,var(--isa-profile-accent) 66%,#8d76bd))!important;color:#fff!important;box-shadow:0 7px 15px color-mix(in srgb,var(--isa-profile-accent) 25%,transparent)!important}
#socialPanel.isa-profile-theme-live .social-brand span,#familySocialOverlay.isa-profile-theme-live .social-brand span,
#socialPanel.isa-profile-theme-live .nuvem-share-title,#familySocialOverlay.isa-profile-theme-live .nuvem-share-title{color:color-mix(in srgb,var(--isa-profile-accent) 76%,#d94d9d)!important}
#socialPanel.isa-profile-theme-live .social-status-strip>* ,#familySocialOverlay.isa-profile-theme-live .social-status-strip>*{border-color:color-mix(in srgb,var(--isa-profile-accent) 28%,#eee)!important}
...
@media(max-width:760px){.isa-theme-color-row{align-items:flex-start}#socialPanel.isa-profile-theme-live,#familySocialOverlay.isa-profile-theme-live{background:linear-gradient(150deg,color-mix(in srgb,var(--isa-profile-accent) 14%,#fff8fc),#f6f3ff 58%,#eef8ff)!important}}
...
function apply(){
  css();const root=overlay();if(!root)return false;const c=accent();
  root.style.setProperty('--isa-profile-accent',c);root.style.setProperty('--isa-profile-soft',mix(c,.18));root.style.setProperty('--isa-profile-veil',mix(c,.10));root.classList.add('isa-profile-theme-live');root.dataset.profileThemeColor=c;
  const host=profileHost(),side=host?.closest('.social-side');if(side){side.classList.add('isa-profile-themed');side.classList.remove('isa-profile-bg');side.style.removeProperty('background-image');side.style.removeProperty('background-position')}
  return true
}
...
bindLiveTheme();new MutationObserver(scan).observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('isa:friend-access-valid',()=>setTimeout(()=>loadState().then(scan),100));document.addEventListener('isa:friend-portal-entered',()=>setTimeout(()=>loadState().then(scan),160));document.addEventListener('click',e=>{if(e.target.closest?.('#socialEditProfile,#fsEditProfile,#socialNav,#friendSocialBtn'))setTimeout(()=>{loadState().then(scan)},60)},true);setTimeout(()=>loadState().then(scan),350);setTimeout(scan,1000)
