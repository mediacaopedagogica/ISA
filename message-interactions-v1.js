...
// Reação só é permitida nas mensagens recebidas. Mensagem própria mantém as ações do núcleo.
...
function ensureCss(){if($('messageInteractionsCss'))return;const l=document.createElement('link');l.id='messageInteractionsCss';l.rel='stylesheet';l.href='./message-interactions-v1.css?v=4-bubble-edge';document.head.appendChild(l)}
...
function showPicker(anchor,id,external,row){if(isMine(row))return;ensurePicker();const p=$('miReactionPicker'),r=anchor.getBoundingClientRect();p.dataset.messageId=id;p.dataset.external=external?'1':'0';p.classList.add('show');const w=Math.min(p.offsetWidth||280,innerWidth-12),h=p.offsetHeight||48;let left=r.right+7,top=r.top+r.height/2-h/2;if(left+w>innerWidth-6)left=Math.max(6,r.left-w-7);if(left<6)left=Math.max(6,Math.min(innerWidth-w-6,r.left+r.width/2-w/2));top=Math.max(6,Math.min(innerHeight-h-6,top));p.style.left=`${left}px`;p.style.top=`${top}px`}
...
function showReceivedMenu(anchor,row){
  ...
  let left=r.right+7;if(left+w>innerWidth-6)left=Math.max(6,r.left-w-7);if(left<6)left=Math.max(6,Math.min(innerWidth-w-6,r.left+r.width/2-w/2));let top=r.top;if(top+h>innerHeight-6)top=Math.max(6,innerHeight-h-6);m.style.left=`${left}px`;m.style.top=`${top}px`
}
...
function renderReactionSummary(row,items,mine){const b=bubble(row);if(!b)return;let box=b.querySelector(':scope > .mi-reaction-summary');if(!box){box=document.createElement('div');box.className='mi-reaction-summary';b.appendChild(box)}const grouped=new Map();for(const x of items){const g=grouped.get(x.reaction)||{count:0,mine:false};g.count++;if(x.member_id===mine)g.mine=true;grouped.set(x.reaction,g)}box.innerHTML=[...grouped].map(([r,g])=>`<span class="mi-reaction-badge ${g.mine?'mine':''}">${r}${g.count>1?` ${g.count}`:''}</span>`).join('');box.style.display=grouped.size?'flex':'none'}
...
function bindReceivedBubble(row,external){
  ...
  const ignore=e=>!!e.target.closest('button,a,input,video,audio')||!!String(window.getSelection?.()||'')
  b.addEventListener('click',e=>{
    if(ignore(e))return
    e.preventDefault();e.stopImmediatePropagation()
    const now=Date.now()
    if(now-last<340){clearTimeout(singleTimer);last=0;hideMenu();showPicker(b,id,external,row);return}
    last=now;clearTimeout(singleTimer);singleTimer=setTimeout(()=>{last=0;showReceivedMenu(b,row)},255)
  },true)
  b.addEventListener('dblclick',e=>{if(ignore(e))return;e.preventDefault();e.stopImmediatePropagation();clearTimeout(singleTimer);last=0;hideMenu();showPicker(b,id,external,row)},true)
  b.addEventListener('contextmenu',e=>{if(ignore(e))return;e.preventDefault();e.stopImmediatePropagation();clearTimeout(singleTimer);last=0;showReceivedMenu(b,row)},true)
}
function decorateRow(row,external){const id=messageId(row);if(!id)return;row.dataset.messageId=id;const b=bubble(row);if(b)b.style.position='relative';if(!isMine(row))bindReceivedBubble(row,external);if(imageEl(row))imageEl(row).classList.add('mi-image-action')}
async function hydrateExternalIds(){const box=$('friendMessages'),conv=window.__ISA_FRIEND_GET_ACTIVE_CONVERSATION_ID__?.();if(!box||!conv||!friendToken)return[];const rows=[...box.querySelectorAll('.friend-msg')];if(!rows.length)return[];if(rows.every(messageId))return rows;const r=await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/rpc/friend_portal_messages`,{method:'POST',headers:{apikey:CONFIG.SUPABASE_KEY,Authorization:`Bearer ${CONFIG.SUPABASE_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({p_token:friendToken,p_conversation_id:conv}),cache:'no-store'});const list=await r.json().catch(()=>[]);if(!r.ok)return rows;rows.forEach((row,i)=>{if(list?.[i]?.id)row.dataset.messageId=list[i].id});return rows}
...
setTimeout(()=>{refresh(false);refresh(true)},350);setInterval(()=>{if(document.visibilityState==='visible'){if($('chatPanel')&&!$('chatPanel').classList.contains('hidden'))refresh(false);if($('friendThread')&&!$('friendThread').classList.contains('hidden'))refresh(true)}},4200)
