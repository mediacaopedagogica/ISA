...
import { CONFIG } from './config.js?v=20260910-super-pais'
...
let me=null,members=[],settings=new Map(),mounted=false
...
async function identity(){if(me)return me;const {data:{user}}=await db.auth.getUser();if(!user)return null;const {data}=await db.from('family_members').select('id,family_id,display_name,relationship_label,role').eq('auth_user_id',user.id).eq('active',true).maybeSingle();me=data;return me}
...
function renderViewer(viewerId){
  ...
  const chosen=effective(viewer),setting=settings.get(viewer.id),max=setting?.max_people||Math.min(8,members.length)
  box.innerHTML=`<div class="spv-options">${members.filter(x=>x.id!==viewer.id).map(t=>{const hard=allowedByHardRule(viewer,t);return `<label class="spv-choice ${hard?'':'locked'}"><input type="checkbox" data-spv-member="${t.id}" ${chosen.has(t.id)&&hard?'checked':''} ${hard?'':'disabled'}><span>${hard?'👤':'🔒'} ${t.display_name}</span></label>`}).join('')}</div><div class="spv-locknote">🔒 Combinações restritas continuam bloqueadas mesmo que alguém tente alterá-las. Ex.: Elion × Evalda/Vânia/Silvane.</div><div class="spv-bottom"><label>Máximo de pessoas em grupos<select id="spvMax">${Array.from({length:Math.min(12,members.length)},(_,i)=>i+2).map(n=>`<option value="${n}" ${Number(max)===n?'selected':''}>Até ${n} pessoas</option>`).join('')}</select></label><button id="spvSave" class="spv-save" type="button">Salvar para ${viewer.display_name}</button></div>`
...
async function save(viewer){
  ...
  settings.set(viewer.id,row);toast(`Conversas de ${viewer.display_name} atualizadas 💜`)
}
function render(){
  const panel=$('parentsPanel');if(!panel||mounted)return;mounted=true;style()
  const card=document.createElement('section');card.id='superPaisVisibilityCard';card.className='spv-card';card.innerHTML=`<div class="spv-head"><div><h3>👑 Quem aparece no chat</h3><small>Escolha, perfil por perfil, quais familiares podem aparecer e o limite de participantes.</small></div></div><div id="spvPersonTabs" class="spv-person-tabs"></div><div id="spvEditor" class="spv-empty">Carregando familiares…</div>`
  const anchor=$('permissionsTable')?.parentElement||panel;anchor.appendChild(card)
  const tabs=$('spvPersonTabs');tabs.innerHTML=members.map((m,i)=>`<button type="button" class="spv-person ${i===0?'active':''}" data-spv-viewer="${m.id}">${m.display_name}</button>`).join('')
  ...
  if(members[0])renderViewer(members[0].id)
}
async function start(){try{if(await load())render()}catch(e){console.warn('Super Pais:',e)}}
start();document.addEventListener('isa:parent-panel-opened',start)
window.__ISA_SUPER_PAIS_VISIBILITY__={start,renderViewer}
