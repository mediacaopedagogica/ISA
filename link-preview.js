...
function ensureUi(){
  ...
  @media(max-width:760px){.isa-video-drawer{left:10px;right:10px;top:auto;bottom:10px;width:auto;max-height:78vh}}
  `;document.head.appendChild(s)
  const d=document.createElement('aside');d.id='isaVideoDrawer';d.className='isa-video-drawer';d.innerHTML='<div class="isa-video-head"><span>🎬</span><strong>Assistir sem sair do Cantinho</strong><button class="isa-video-close" type="button" aria-label="Fechar">✕</button></div><div id="isaVideoFrame" class="isa-video-frame"></div><a id="isaVideoOriginal" class="isa-video-url" target="_blank" rel="noopener noreferrer">Abrir link original ↗</a>';document.body.appendChild(d);d.querySelector('.isa-video-close').onclick=closeVideo
}
...
function splitTextNode(node){
  ...
  while((m=URL_RE.exec(text))){
    ...
    if(parseVideo(url)){const spacer=document.createTextNode(' '),b=document.createElement('button');b.type='button';b.className='isa-video-open';b.textContent='▶ Assistir aqui';b.dataset.videoUrl=url;b.onclick=e=>{e.preventDefault();e.stopPropagation();openVideo(url)};frag.append(spacer,b)}
...
function candidateRoots(){return [...document.querySelectorAll('#messages .bubble > div:not(.photo-loading):not(.photo-expired):not([class*="audio"]), #friendMessages .friend-bubble > div:not(.friend-photo-wrap):not(.friend-photo-note):not(.friend-audio-wrap)')]}
function linkify(){
