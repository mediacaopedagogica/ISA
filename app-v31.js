...
${suffix}`;
  ...
  async function __isaCore31() {
    ...
    function isOnline(memberId) {
      ...
      return !!(p?.online && Date.now() - new Date(p.last_seen_at).getTime() < 95e3);
    }
    ...
    async function upsertPresence(online) {
      if (!me) return;
      await supabase.from("presence_state").upsert({ member_id: me.id, online, last_seen_at: (/* @__PURE__ */ new Date()).toISOString(), device_label: navigator.userAgent.includes("Mobile") ? "Celular" : "Computador" }, { onConflict: "member_id" }).catch(() => {
      });
    }
    ...
    async function bindGlobalRealtime() {
      ...
      globalChannel = supabase.channel(`family-ui-${me.id}`).on("postgres_changes", { event: "*", schema: "public", table: "messages" }, async (payload) => {
        await loadConversations();
        if (activeConversation?.id === (payload.new?.conversation_id || payload.old?.conversation_id)) {
          await loadMessages(activeConversation.id);
          renderTimeline();
        }
