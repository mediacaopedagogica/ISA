...
function profileFor(memberId){return familyProfiles.find(p=>String(p.member_id)===String(memberId))||null}
function memberFor(memberId){if(String(me?.id)===String(memberId))return me;return profileFor(memberId)?.family_members||null}
function avatarSpec(member,p){
  if(p?.social_avatar_ref)return{ref:p.social_avatar_ref,bucket:'social-media'}
  const ref=member?.avatar_ref||''
  if(ref.startsWith('emoji:'))return{emoji:ref.slice(6)}
  return ref?{ref,bucket:'profile-avatars'}:{}
}
function avatarHtml(member,p=undefined){
  const spec=avatarSpec(member,p),name=displayName(member,p),initial=(name||'?').trim().charAt(0).toUpperCase()||'💜'
  if(spec.emoji)return `<div class="social-avatar">${esc(spec.emoji)}</div>`
  if(spec.ref)return `<div class="social-avatar" data-social-avatar-ref="${esc(spec.ref)}" data-social-avatar-bucket="${esc(spec.bucket)}" data-social-avatar-name="${esc(name)}">${esc(initial)}</div>`
  return `<div class="social-avatar">${esc(initial)}</div>`
}
async function avatarObjectUrl(ref,bucket){
  if(!ref)return'';const key=`${bucket}:${ref}`,cached=avatarUrlCache.get(key);if(cached)return cached
  const {data,error}=await db.storage.from(bucket).download(ref);if(error||!data)return''
  const url=URL.createObjectURL(data);avatarUrlCache.set(key,url);return url
}
async function hydrateSocialAvatars(root=document){
  const els=[...(root?.querySelectorAll?.('[data-social-avatar-ref]')||[])]
  await Promise.allSettled(els.map(async el=>{
    const ref=el.dataset.socialAvatarRef||'',bucket=el.dataset.socialAvatarBucket||'profile-avatars';if(!ref)return
    const url=await avatarObjectUrl(ref,bucket);if(!url||!el.isConnected||el.dataset.socialAvatarRef!==ref)return
    const current=el.querySelector('img');if(current?.dataset.avatarRef===ref)return
    el.innerHTML=`<img src="${url}" data-avatar-ref="${esc(ref)}" alt="Foto de ${esc(el.dataset.socialAvatarName||'perfil')}">`
  }))
}
...
function injectUi(){
  if(!$('socialNav')){
    const nav=document.querySelector('.nav-tabs');if(nav){const btn=document.createElement('button');btn.className='nav-btn';btn.id='socialNav';btn.type='button';btn.innerHTML='🌸 <span>Nossa Rede</span>';const calendar=nav.querySelector('[data-tab="calendar"]');calendar?.after(btn);if(!btn.parentNode)nav.appendChild(btn)}
  }
  if(!$('socialPanel')){
    if(!$('socialNetworkBaseCss')){const css=document.createElement('link');css.id='socialNetworkBaseCss';css.rel='stylesheet';css.href='./social-network.css?v=2';document.head.appendChild(css)}
    const content=document.querySelector('main.content');if(!content)return false
    const panel=document.createElement('section');panel.id='socialPanel';panel.className='social-panel hidden';panel.innerHTML=`
      <div class="social-shell">
        <aside class="social-card social-side"><div class="social-brand">Nossa <span>Rede</span> ✨</div><div id="socialMyProfile"></div></aside>
...
async function openSocial(){
  injectUi();allMainSections().forEach(s=>s.classList.add('hidden'));$('socialPanel')?.classList.remove('hidden')
  document.querySelectorAll('.nav-btn').forEach(b=>b.classList.toggle('active',b.id==='socialNav'))
  $('mainView')?.classList.add('social-mode')
  document.dispatchEvent(new CustomEvent('isa:social-opened'))
  try{await loadAll(true)}catch(e){console.error(e);toast('Não foi possível atualizar a Nossa Rede.')}
  window.__ISA_PROFILE_THEME_V2__?.loadState?.().then(()=>window.__ISA_PROFILE_THEME_V2__?.apply?.()).catch(()=>{})
}
function leaveSocial(){$('socialPanel')?.classList.add('hidden');$('mainView')?.classList.remove('social-mode')}

function wireUi(){
  const root=$('socialPanel');if(!root||root.dataset.socialCoreWired==='1')return;root.dataset.socialCoreWired='1'
  ...
  $('socialRefresh').onclick=()=>loadAll(true).catch(()=>toast('Não foi possível atualizar agora.'))
  ...
  document.querySelectorAll('.social-theme').forEach(b=>b.onclick=()=>{activeTheme=b.dataset.theme;document.querySelectorAll('.social-theme').forEach(x=>x.classList.toggle('active',x.dataset.theme===activeTheme));document.dispatchEvent(new CustomEvent('isa:profile-theme-preview',{detail:{theme:activeTheme}}))})
  $('socialProfileModal')?.addEventListener('click',e=>{if(e.target.id==='socialProfileModal')closeProfileModal()})
}

function renderPreview(){const box=$('socialPreview');if(!box)return;box.innerHTML='';for(const file of selectedFiles){const url=URL.createObjectURL(file),d=document.createElement('div');d.className='social-preview-item';d.innerHTML=file.type.startsWith('video/')?`<video src="${url}" muted></video>`:`<img src="${url}" alt="Prévia">`;box.appendChild(d)}}
function openProfileModal(){if(!profile||!me)return;$('socialName').value=profile.display_name||me.display_name;$('socialBio').value=profile.bio||'';$('socialStatus').value=profile.status_text||'';$('socialMood').value=profile.mood_emoji||'💜';$('socialActivity').value=profile.activity_label||'';activeTheme=profile.theme||'lilac';document.querySelectorAll('.social-theme').forEach(x=>x.classList.toggle('active',x.dataset.theme===activeTheme));$('socialProfileModal').classList.add('show');document.dispatchEvent(new CustomEvent('isa:social-profile-editor-opened'))}
...
async function saveProfile(){
  ...
  const {data,error}=await db.from('social_profiles').upsert(payload,{onConflict:'member_id'}).select().single();if(error)return toast('Não foi possível salvar o perfil.')
...
async function publishPost(){
  ...
  try{
    ...
    const mediaRows=[];for(let i=0;i<selectedFiles.length;i++){const f=selectedFiles[i],path=await uploadFile(f,post.id,i);mediaRows.push({post_id:post.id,media_type:f.type.startsWith('video/')?'video':'image',storage_path:path,mime_type:f.type,sort_order:i})}
    if(mediaRows.length){const {error:meErr}=await db.from('social_post_media').insert(mediaRows);if(meErr)throw meErr}
...
async function loadProfiles(){
  const {data,error}=await db.from('social_profiles').select('member_id,family_id,display_name,bio,status_text,mood_emoji,activity_label,theme,theme_color,social_avatar_ref,cover_ref,updated_at,family_members!social_profiles_member_id_fkey(id,display_name,relationship_label,avatar_ref)').eq('family_id',me.family_id).order('updated_at',{ascending:false});if(error)throw error
  familyProfiles=data||[];const mine=familyProfiles.find(p=>String(p.member_id)===String(me.id));if(mine){profile={...(profile||{}),...mine};activeTheme=profile.theme||activeTheme||'lilac'}
}
async function loadPosts(){
  const {data,error}=await db.from('social_posts').select('id,family_id,author_id,caption,location_label,created_at,updated_at,author:family_members!social_posts_author_id_fkey(id,display_name,relationship_label,avatar_ref),media:social_post_media(id,media_type,storage_path,mime_type,sort_order),reactions:social_post_reactions(member_id,reaction),comments:social_post_comments(id,author_id,body,created_at,deleted_at,author:family_members!social_post_comments_author_id_fkey(id,display_name,avatar_ref))').eq('family_id',me.family_id).is('deleted_at',null).order('created_at',{ascending:false}).limit(40);if(error)throw error
  posts=data||[];for(const p of posts){p.media=(p.media||[]).sort((a,b)=>a.sort_order-b.sort_order);p.comments=(p.comments||[]).filter(c=>!c.deleted_at).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));for(const m of p.media)m.url=await mediaUrl(m.storage_path)}
}
function profileSignature(){return stableString({id:profile?.member_id,name:profile?.display_name,bio:profile?.bio,status:profile?.status_text,mood:profile?.mood_emoji,activity:profile?.activity_label,theme:profile?.theme,themeColor:profile?.theme_color,avatar:profile?.social_avatar_ref,memberAvatar:me?.avatar_ref})}
function statusSignature(){return stableString(familyProfiles.map(p=>[p.member_id,p.display_name,p.status_text,p.mood_emoji,p.activity_label,p.social_avatar_ref,p.family_members?.avatar_ref]))}
function familySignature(){return stableString(familyProfiles.map(p=>[p.member_id,p.display_name,p.activity_label,p.status_text,p.social_avatar_ref,p.family_members?.avatar_ref]))}
function feedSignature(){return stableString(posts.map(p=>[p.id,p.caption,p.location_label,p.created_at,p.updated_at,p.author_id,(p.media||[]).map(m=>[m.id,m.storage_path,m.sort_order]),(p.reactions||[]).map(r=>[r.member_id,r.reaction]),(p.comments||[]).map(c=>[c.id,c.author_id,c.body,c.created_at])]))}
async function loadAll(forceMedia=false){
  if(loadPromise)return loadPromise
  loadPromise=(async()=>{
    if(!me)await identity();if(!me)return false
    if(forceMedia===true)mediaUrlCache.clear()
    await Promise.all([loadProfiles(),loadPosts()]);lastLoadAt=Date.now();renderAll();return true
  })().finally(()=>loadPromise=null)
  return loadPromise
}
function renderAll(){
  const a=profileSignature(),b=statusSignature(),c=familySignature(),d=feedSignature()
  if(a!==sigProfile){sigProfile=a;renderMyProfile()}
  if(b!==sigStatus){sigStatus=b;renderStatusStrip()}
  if(c!==sigFamily){sigFamily=c;renderFamily()}
  if(d!==sigFeed){sigFeed=d;renderFeed()}
  hydrateSocialAvatars($('socialPanel')).catch(()=>{})
  document.dispatchEvent(new CustomEvent('isa:social-rendered'))
  window.__ISA_PROFILE_THEME_V2__?.apply?.();window.__ISA_ENHANCE_NOSSA_REDE__?.();window.__ISA_FAMILY_SOCIAL_EXTRAS__?.scan?.()
}
function renderMyProfile(){
  const box=$('socialMyProfile');if(!box)return;box.innerHTML=`<div class="social-profile-head">${avatarHtml(me,profile)}<div><strong>${esc(displayName(me,profile))}</strong><small>${esc(me.relationship_label||'Família')}</small></div></div><div class="social-bio">${esc(profile?.bio||'Adicione uma bio ao seu perfil ✨')}</div><div class="social-now"><div class="social-pill"><b>${esc(profile?.mood_emoji||'💜')}</b><span>${esc(profile?.status_text||'Sem frase do dia')}</span></div><div class="social-pill">${esc(profile?.activity_label||'✨ Livre agora')}</div></div><button id="socialEditProfile" class="social-btn primary" style="width:100%" type="button">Personalizar perfil</button>`;$('socialEditProfile').onclick=openProfileModal;hydrateSocialAvatars(box).catch(()=>{})
}
function renderStatusStrip(){const box=$('socialStatusStrip');if(!box)return;const withStatus=familyProfiles.filter(p=>p.status_text||p.activity_label).slice(0,12);box.innerHTML=withStatus.length?withStatus.map(p=>`<div class="social-status-card"><b>${esc(p.mood_emoji||'💜')}</b><strong>${esc(displayName(p.family_members,p))}</strong><small>${esc(p.activity_label||p.status_text||'')}</small></div>`).join(''):''}
function renderFamily(){const box=$('socialFamilyList');if(!box)return;box.innerHTML=familyProfiles.map(p=>`<div class="social-family-item">${avatarHtml(p.family_members,p)}<div><strong>${esc(displayName(p.family_members,p))}</strong><small>${esc(p.activity_label||p.status_text||'Sem atualização')}</small></div></div>`).join('')||'<div class="social-empty">A família vai aparecer aqui 💜</div>';hydrateSocialAvatars(box).catch(()=>{})}
...
function wireFeed(){document.querySelectorAll('[data-carousel-prev]').forEach(b=>b.onclick=()=>setCarousel(b.dataset.carouselPrev,-1));document.querySelectorAll('[data-carousel-next]').forEach(b=>b.onclick=()=>setCarousel(b.dataset.carouselNext,1));document.querySelectorAll('[data-react]').forEach(b=>b.onclick=()=>react(b.dataset.postId,b.dataset.react));document.querySelectorAll('[data-more-react]').forEach(b=>b.onclick=()=>{const r=prompt('Escolha uma reação: '+reactions.join(' '),'💜');if(r&&reactions.includes(r))react(b.dataset.moreReact,r)});document.querySelectorAll('[data-comment-form]').forEach(f=>f.onsubmit=e=>{e.preventDefault();const inp=f.querySelector('input');addComment(f.dataset.commentForm,inp.value)});document.querySelectorAll('[data-post-menu]').forEach(b=>b.onclick=()=>{const action=prompt('Digite E para editar ou X para excluir','E');if(action?.toUpperCase()==='E')editPost(b.dataset.postMenu);if(action?.toUpperCase()==='X')deletePost(b.dataset.postMenu)})}

function bindRealtime(){
  if(realtimeBound||!me?.family_id)return;realtimeBound=true;let t=0;const refresh=()=>{clearTimeout(t);t=setTimeout(()=>{if(!$('socialPanel')?.classList.contains('hidden'))loadAll().catch(()=>{})},220)}
  try{db.channel(`social-stable-${me.family_id}`).on('postgres_changes',{event:'*',schema:'public',table:'social_profiles',filter:`family_id=eq.${me.family_id}`},refresh).on('postgres_changes',{event:'*',schema:'public',table:'social_posts',filter:`family_id=eq.${me.family_id}`},refresh).subscribe()}catch(e){console.warn('Nossa Rede realtime:',e)}
}
async function boot(){if(booted)return;booted=true;if(!injectUi())return;const m=await identity().catch(()=>null);if(!m)return;await loadAll().catch(console.error);bindRealtime();document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&!$('socialPanel')?.classList.contains('hidden')&&Date.now()-lastLoadAt>30000)loadAll().catch(()=>{})})}
