...
later(730,'./approved-conversation-stability-v1.js?v=2-single-shell-safe',()=>window.__ISA_STABILIZE_APPROVED_CONVERSATIONS__?.())
later(760,'./social-nav-guard.js?v=10-single-shell',()=>{window.__ISA_ENSURE_SOCIAL_NAV__?.();window.__ISA_ENSURE_PROFILE_MENU__?.();window.__ISA_PERSONAL_NAV_SYNC__?.()})
later(840,'./nossa-rede-v4.js?v=7-single-shell-open',()=>{window.__ISA_ENHANCE_NOSSA_REDE__?.();window.__ISA_SOCIAL_PRIVACY__?.apply?.();window.__ISA_FAMILY_SOCIAL_EXTRAS__?.scan?.(true);window.__ISA_NOSSA_REDE_BIRTHDAYS__?.refresh?.();window.__ISA_PERSONAL_NAV_SYNC__?.()})
